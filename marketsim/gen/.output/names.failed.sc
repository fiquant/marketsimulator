...
package event
{
    ...
    def Every(/** interval of time between two events */ intervalFunc = math.random.expovariate(1.0)) : IEvent // defined at .output\names.sc: 25.5
    
    /** Event that once at *delay*
     */
    @python.intrinsic("event._After_Impl")
    def After(/** when the event should be fired */ delay = constant(10.0)) : IEvent // defined at .output\names.sc: 30.5
...
package veusz
{
    ...
    def Graph(name = "graph") : IGraph // defined at .output\names.sc: 40.5
...
package ops
{
    ...
    @label = "-%(x)s"
    def Negate(x = constant(1.0)) : IObservable[Float] // defined at .output\names.sc: 50.5
    
    @label = "({%(x)s}{{symbol}}{%(y)s})"
    @python.intrinsic.observable("ops._Add_Impl")
    @symbol = "+"
    def Add(x = constant(1.0),
            y = constant(1.0)) : IObservable[Float] // defined at .output\names.sc: 54.5
    
    @label = "({%(x)s}{{symbol}}{%(y)s})"
    @python.intrinsic.observable("ops._Less_Impl")
    @symbol = "<"
    def Less(x = constant(1.0),
             y = constant(1.0)) : IObservable[Boolean] // defined at .output\names.sc: 60.5
    ...
    def Condition_Float(cond = true() : IFunction[Boolean],
                        ifpart = constant(1.0),
                        elsepart = constant(1.0)) : IObservable[Float] // defined at .output\names.sc: 72.5
    ...
    def Condition_Side(cond = true() : IFunction[Boolean],
                       ifpart = side.Sell(),
                       elsepart = side.Buy()) : IObservable[Side] // defined at .output\names.sc: 84.5
...
package math
{
    package random
    {
        ...
        def gammavariate(Alpha = 1.0,
                         Beta = 1.0) : () => Float // defined at .output\names.sc: 132.9
        
        /** Normal distribution
         */
        @python.random()
        def normalvariate(/** |mu| is the mean                  */ Mu = 0.0,
                          /** |sigma| is the standard deviation */ Sigma = 1.0) : () => Float // defined at .output\names.sc: 146.9
        
        /** Pareto distribution
         */
        @python.random()
        def paretovariate(/** |alpha| is the shape parameter*/ Alpha = 1.0) : () => Float // defined at .output\names.sc: 152.9
        ...
        def triangular(Low = 0.0,
                       High = 1.0,
                       Mode = 0.5) : () => Float // defined at .output\names.sc: 157.9
        ...
        def vonmisesvariate(/** |mu| is the mean angle, expressed in radians between 0 and 2|pi|*/ Mu = 0.0,
                            /** |kappa| is the concentration parameter, which must be greater than or equal to zero.
                              ...
                              *      to a uniform random angle over the range 0 to 2|pi|        */ Kappa = 0.0) : () => Float // defined at .output\names.sc: 170.9
        ...
        def uniform(Low = -10.0,
                    High = 10.0) : () => Float // defined at .output\names.sc: 178.9
        
        /** Weibull distribution
         */
        @python.random()
        def weibullvariate(/** |alpha| is the scale parameter */ Alpha = 1.0,
                           /** |beta| is the shape parameter  */ Beta = 1.0) : () => Float // defined at .output\names.sc: 189.9
         ...
         */
        @python.random()
        def expovariate(/** |lambda| is 1.0 divided by the desired mean. It should be greater zero.*/ Lambda = 1.0) : () => Float // defined at .output\names.sc: 195.9
        ...
        def lognormvariate(Mu = 0.0,
                           Sigma = 1.0) : () => Float // defined at .output\names.sc: 202.9
    ...
    package Cumulative
    {
         ...
         */
        @label = "RSD{{suffix}}"
        def RelStdDev(/** observable data source */ source = const()) = (source-Avg(source))/StdDev(source) // defined at .output\names.sc: 227.9
        
        /** Cumulative variance
         */
        @python.intrinsic("moments.cmv.Variance_Impl")
        @label = "\\sigma^2{{suffix}}"
        def Var(/** observable data source */ source = const()) : () => Float // defined at .output\names.sc: 232.9
        
        /** Cumulative average
         */
        @python.intrinsic("moments.cma.CMA_Impl")
        @label = "Avg{{suffix}}"
        def Avg(/** observable data source */ source = const()) : () => Float // defined at .output\names.sc: 238.9
        ...
        def MinEpsilon(/** observable data source */ source = constant(),
                       /** tolerance step         */ epsilon = constant(0.01)) : IObservable[Float] // defined at .output\names.sc: 244.9
        ...
        def StdDev(/** observable data source */ source = const()) = Sqrt(Var(source)) // defined at .output\names.sc: 262.9
    ...
    package rsi
    {
        ...
        def Raw(/** observable data source */ source = const(),
                ...
                /** alpha parameter for EWMA */ alpha = 0.015) = EW.Avg(UpMovements(source,timeframe),alpha)/EW.Avg(DownMovements(source,timeframe),alpha) // defined at .output\names.sc: 272.9
    ...
    package macd
    {
        ...
        def MACD(/** source */ x = const(),
                 ...
                 /** short period */ fast = 12.0) = EW.Avg(x,2.0/(fast+1))-EW.Avg(x,2.0/(slow+1)) // defined at .output\names.sc: 284.9
        ...
        def Signal(/** source */ x = const(),
                   ...
                   /** discretization step */ step = 1.0) = EW.Avg(observable.OnEveryDt(step,MACD(x,slow,fast)),2/(timeframe+1)) // defined at .output\names.sc: 291.9
        ...
        def Histogram(/** source */ x = const(),
                      ...
                      /** short period */ fast = 12.0,
                      /** signal period */ timeframe = 9.0,
                      /** discretization step */ step = 1.0) = MACD(x,slow,fast)-Signal(x,slow,fast,timeframe,step) // defined at .output\names.sc: 300.9
    ...
    package EW
    {
        ...
        def Avg(/** observable data source */ source = const(),
                /** alpha parameter */ alpha = 0.015) : IDifferentiable // defined at .output\names.sc: 315.9
        ...
        def Var(/** observable data source */ source = const(),
                /** alpha parameter */ alpha = 0.015) : () => Float // defined at .output\names.sc: 322.9
        
        /** Exponentially weighted moving standard deviation
         */
        @label = "\\sqrt{\\sigma^2{{suffix}}}"
        def StdDev(/** observable data source */ source = const(),
                   /** alpha parameter */ alpha = 0.015) = Sqrt(Var(source,alpha)) // defined at .output\names.sc: 329.9
        
        /** Exponentially weighted moving relative standard deviation
         */
        @label = "RSD{{suffix}}"
        def RelStdDev(/** observable data source */ source = const(),
                      /** alpha parameter */ alpha = 0.015) = (source-Avg(source,alpha))/StdDev(source,alpha) // defined at .output\names.sc: 335.9
    ...
    package Moving
    {
        ...
        def Min(/** observable data source */ source = constant(),
                /** sliding window size    */ timeframe = 100.0) : IObservable[Float] // defined at .output\names.sc: 347.9
        
        /** Simple moving relative standard deviation
         */
        @label = "RSD{{suffix}}"
        def RelStdDev(/** observable data source */ source = const(),
                      /** sliding window size    */ timeframe = 100.0) = (source-Avg(source,timeframe))/StdDev(source,timeframe) // defined at .output\names.sc: 354.9
        ...
        @label = "\\sigma^2{{suffix}}"
        def Var(/** observable data source */ source = const(),
                /** sliding window size    */ timeframe = 100.0) = math.Max(const(0),Avg(source*source,timeframe)-Sqr(Avg(source,timeframe))) // defined at .output\names.sc: 360.9
        ...
        @label = "Avg{{suffix}}"
        def Avg(/** observable data source */ source = const(),
                /** sliding window size    */ timeframe = 100.0) : () => Float // defined at .output\names.sc: 374.9
        
        /** Simple moving standard deviation
         */
        @label = "\\sqrt{\\sigma^2{{suffix}}}"
        def StdDev(/** observable data source */ source = const(),
                   /** sliding window size    */ timeframe = 100.0) = Sqrt(Var(source)) // defined at .output\names.sc: 381.9
    ...
    @label = "min{%(x)s, %(y)s}"
    def Min(x = constant(),
            y = constant()) = if x<y then x else y // defined at .output\names.sc: 389.5
    ...
    def DownMovements(/** observable data source */ source = const(),
                      /** lag size */ timeframe = 10.0) = observable.Float(Max(const(0.0),Lagged(source,timeframe)-source)) // defined at .output\names.sc: 397.5
    ...
    @python.mathops("atan")
    def Atan(x = constant(0.0)) : () => Float // defined at .output\names.sc: 403.5
    ...
    def Lagged(/** observable data source */ source = const(),
               /** lag size */ timeframe = 10.0) : IObservable[Float] // defined at .output\names.sc: 410.5
    ...
    def UpMovements(/** observable data source */ source = const(),
                    /** lag size */ timeframe = 10.0) = observable.Float(Max(const(0.0),source-Lagged(source,timeframe))) // defined at .output\names.sc: 425.5
    ...
    @python.observable()
    @label = "{%(x)s}^2"
    def Sqr(x = constant()) = x*x // defined at .output\names.sc: 431.5
    
    /** Log returns
     */
    @label = "LogReturns_{%(timeframe)s}(%(x)s)"
    def LogReturns(/** observable data source */ x = const(),
                   /** lag size */ timeframe = 10.0) = Log(x/Lagged(x,timeframe)) // defined at .output\names.sc: 438.5
    ...
    @python.mathops("sqrt")
    @label = "\\sqrt{%(x)s}"
    def Sqrt(x = constant(1.0)) : () => Float // defined at .output\names.sc: 444.5
    ...
    def RSI(/** asset price in question  */ book = orderbook.OfTrader(),
            /** lag size */ timeframe = 10.0,
            /** alpha parameter for EWMA */ alpha = 0.015) = 100.0-100.0/(1.0+rsi.Raw(orderbook.MidPrice(book),timeframe,alpha)) // defined at .output\names.sc: 452.5
    ...
    @python.mathops("exp")
    @label = "e^{%(x)s}"
    def Exp(x = constant(1.0)) : () => Float // defined at .output\names.sc: 459.5
    ...
    @python.mathops("log")
    @label = "log(%(x)s)"
    def Log(x = constant(1.0)) : () => Float // defined at .output\names.sc: 467.5
    ...
    def RandomWalk(/** initial value of the signal */ initialValue = 0.0,
                   ...
                   name = "-random-") : IObservable[Float] // defined at .output\names.sc: 475.5
    ...
    @label = "\\frac{d%(x)s}{dt}"
    def Derivative(x = math.EW.Avg() : IDifferentiable) : () => Float // defined at .output\names.sc: 484.5
    ...
    def Pow(base = constant(1.0),
            power = constant(1.0)) : () => Float // defined at .output\names.sc: 491.5
...
package order
{
    ...
    package _curried
    {
        ...
        def side_ImmediateOrCancel(/** factory for underlying orders */ proto = side.Limit()) : (() => .Side) => .IOrderGenerator // defined at .output\names.sc: 642.9
        ...
        def volume_price_Limit(/** function defining side of orders to create */ side = side.Sell()) : (() => .Float) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 654.9
        ...
        def sidevolume_Iceberg(/** maximal size of order to send */ lotSize = constant(10.0),
                               /** underlying orders to create */ proto = side_volume.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 663.9
        ...
        def side_price_StopLoss(/** maximal acceptable loss factor */ maxloss = constant(0.1),
                                /** underlying orders to create */ proto = side.price.Limit()) : (() => .Side) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 682.9
        ...
        def price_Iceberg(/** maximal size of order to send */ lotSize = constant(10.0),
                          /** underlying orders to create */ proto = price.Limit()) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 693.9
         ...
         */
        @python.order.factory.on_proto("ImmediateOrCancel")
        def price_ImmediateOrCancel(/** factory for underlying orders */ proto = price.Limit()) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 724.9
        ...
        def volume_price_WithExpiry(/** expiration period for orders */ expiry = constant(10.0),
                                    /** underlying orders to create */ proto = volume.price.Limit()) : (() => .Float) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 736.9
        ...
        def sideprice_StopLoss(/** maximal acceptable loss factor */ maxloss = constant(0.1),
                               /** underlying orders to create */ proto = side_price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 745.9
        ...
        def volume_price_Peg(proto = volume.price.Limit()) : (() => .Float) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 756.9
        ...
        def side_Iceberg(/** maximal size of order to send */ lotSize = constant(10.0),
                         /** underlying orders to create */ proto = side.Limit()) : (() => .Side) => .IOrderGenerator // defined at .output\names.sc: 766.9
        ...
        def sidevolume_price_WithExpiry(/** expiration period for orders */ expiry = constant(10.0),
                                        /** underlying orders to create */ proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 776.9
        ...
        def volume_Peg(proto = volume.price.Limit()) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 785.9
         ...
         */
        @python.order.factory.on_proto("ImmediateOrCancel")
        def sidevolume_ImmediateOrCancel(/** factory for underlying orders */ proto = side_volume.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 795.9
        ...
        def side_FixedBudget(/** function defining budget on which it may send orders at one time */ budget = constant(1000.0)) : (() => .Side) => .IOrderGenerator // defined at .output\names.sc: 807.9
        ...
        def sideprice_Limit(/** function defining volume of orders to create */ volume = constant(1.0)) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 819.9
         ...
         */
        @python.order.factory.on_proto("Peg")
        def sideprice_Peg(proto = side.price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 828.9
         ...
         */
        @python.order.factory.on_proto("Peg")
        def sidevolume_Peg(proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 838.9
         ...
         */
        @python.order.factory.on_proto("Peg")
        def side_Peg(proto = side.price.Limit()) : (() => .Side) => .IOrderGenerator // defined at .output\names.sc: 848.9
        ...
        def signedVolume_LimitSigned(/** function defining price of orders to create */ price = constant(100.0)) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 858.9
         ...
         */
        @python.order.factory.curried("LimitSigned")
        def price_LimitSigned(/**signed volume*/ signedVolume : () => .Float = constant(1.0)) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def side_price_ImmediateOrCancel(/** factory for underlying orders */ proto = side.price.Limit()) : (() => .Side) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 877.9
         ...
         */
        @python.order.factory.on_proto("Peg")
        def price_Peg(proto = price.Limit()) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 920.9
        ...
        def volume_StopLoss(/** maximal acceptable loss factor */ maxloss = constant(0.1),
                            /** underlying orders to create */ proto = volume.Limit()) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 930.9
         ...
         */
        @python.order.factory.curried("price_Limit")
        def sidevolume_price_Limit() : ((() => .Side),(() => .Float)) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 941.9
         ...
         */
        @python.order.factory.on_proto("ImmediateOrCancel")
        def volume_ImmediateOrCancel(/** factory for underlying orders */ proto = volume.Limit()) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 979.9
        ...
        def volume_FloatingPrice(/** observable defining price of orders to create */ floatingPrice = const(10.0),
                                 /** underlying orders to create */ proto = volume.price.Limit()) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 991.9
        ...
        def volume_Market(/** function defining side of orders to create */ side = side.Sell()) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 1001.9
         ...
         */
        @python.order.factory.on_proto("price_ImmediateOrCancel")
        def volume_price_ImmediateOrCancel(/** factory for underlying orders */ proto = volume.price.Limit()) : (() => .Float) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 1019.9
        ...
        def side_Limit(/** function defining price of orders to create */ price = constant(100.0),
                       /** function defining volume of orders to create */ volume = constant(1.0)) : (() => .Side) => .IOrderGenerator // defined at .output\names.sc: 1031.9
        ...
        def side_FloatingPrice(/** observable defining price of orders to create */ floatingPrice = const(10.0),
                               /** underlying orders to create */ proto = side.price.Limit()) : (() => .Side) => .IOrderGenerator // defined at .output\names.sc: 1051.9
         ...
         */
        @python.order.factory.curried("price_Limit")
        def side_price_Limit(/** function defining volume of orders to create */ volume = constant(1.0)) : (() => .Side) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 1070.9
         ...
         */
        @python.order.factory.curried("Market")
        def side_Market(/** function defining volume of orders to create */ volume = constant(1.0)) : (() => .Side) => .IOrderGenerator // defined at .output\names.sc: 1099.9
        ...
        def sidevolume_FloatingPrice(/** observable defining price of orders to create */ floatingPrice = const(10.0),
                                     /** underlying orders to create */ proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 1116.9
         ...
         */
        @python.order.factory.on_proto("price_ImmediateOrCancel")
        def sidevolume_price_ImmediateOrCancel(/** factory for underlying orders */ proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 1155.9
         ...
         */
        @python.order.factory.curried("Market")
        def sidevolume_Market() : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 1167.9
        ...
        def price_Limit(/** function defining side of orders to create */ side = side.Sell(),
                        /** function defining volume of orders to create */ volume = constant(1.0)) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 1174.9
         ...
         */
        @python.order.factory.curried("Limit")
        def sidevolume_Limit(/** function defining price of orders to create */ price = constant(100.0)) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 1184.9
         ...
         */
        @python.order.factory.on_proto("price_Peg")
        def sidevolume_price_Peg(proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 1203.9
        ...
        def signedVolume_MarketSigned() : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 1213.9
         ...
         */
        @python.order.factory.on_proto("ImmediateOrCancel")
        def sideprice_ImmediateOrCancel(/** factory for underlying orders */ proto = side_price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at .output\names.sc: 1220.9
         ...
         */
        @python.order.factory.on_proto("price_Peg")
        def side_price_Peg(proto = side.price.Limit()) : (() => .Side) => ((() => .Float) => .IOrderGenerator) // defined at .output\names.sc: 1232.9
        ...
        def volume_Limit(/** function defining side of orders to create */ side = side.Sell(),
                         /** function defining price of orders to create */ price = constant(100.0)) : (() => .Float) => .IOrderGenerator // defined at .output\names.sc: 1252.9
    ...
    def Limit(/** function defining side of orders to create */ side = side.Sell(),
              ...
              /** function defining volume of orders to create */ volume = constant(1.0)) : IOrderGenerator // defined at .output\names.sc: 1302.5
    ...
    def MarketSigned(/**signed volume*/ signedVolume : () => .Float = constant(1.0)) : .IOrderGenerator // defined at .output\names.sc: 1313.5
    ...
    def ImmediateOrCancel(/** factory for underlying orders */ proto = Limit()) : IOrderGenerator // defined at .output\names.sc: 1320.5
    ...
    def StopLoss(/** maximal acceptable loss factor */ maxloss = constant(0.1),
                 /** underlying orders to create */ proto = Limit()) : IOrderGenerator // defined at .output\names.sc: 1340.5
    ...
    def FloatingPrice(/** observable defining price of orders to create */ floatingPrice = const(10.0),
                      /** underlying orders to create */ proto = price.Limit()) : IOrderGenerator // defined at .output\names.sc: 1360.5
    ...
    def FixedBudget(/** function defining side of orders to create */ side = side.Sell(),
                    /** function defining budget on which it may send orders at one time */ budget = constant(1000.0)) : IOrderGenerator // defined at .output\names.sc: 1380.5
    ...
    def LimitSigned(/**signed volume*/ signedVolume : () => .Float = constant(1.0),
                    /** function defining price of orders to create */ price = constant(100.0)) : .IOrderGenerator // defined at .output\names.sc: 1393.5
    ...
    def Peg(proto = price.Limit()) : IOrderGenerator // defined at .output\names.sc: 1403.5
...
package strategy
{
    ...
    package side
    {
        ...
        def PairTrading(/** reference to order book for another asset used to evaluate fair price of our asset */ bookToDependOn = orderbook.OfTrader(),
                        ...
                        /** asset in question */ book = orderbook.OfTrader()) = observable.Side(FundamentalValue(orderbook.MidPrice(bookToDependOn)*factor,book)) // defined at .output\names.sc: 1421.9
        
        /** Side function for signal strategy
         */
        @python.observable()
        def Signal(/** signal to be listened to */ signal = constant(0.0),
                   /** threshold when the trader starts to act */ threshold = 0.7) = if signal>threshold then side.Buy() else if signal<0-threshold then side.Sell() else side.Nothing() // defined at .output\names.sc: 1427.9
        ...
        def CrossingAverages(/** parameter |alpha| for exponentially weighted moving average 1 */ alpha_1 = 0.15,
                             ...
                             /** asset in question */ book = orderbook.OfTrader()) = Signal(math.EW.Avg(orderbook.MidPrice(book),alpha_1)-math.EW.Avg(orderbook.MidPrice(book),alpha_2),threshold) // defined at .output\names.sc: 1433.9
        ...
        def TrendFollower(/** parameter |alpha| for exponentially weighted moving average */ alpha = 0.15,
                          /** threshold when the trader starts to act */ threshold = 0.0,
                          /** asset in question */ book = orderbook.OfTrader()) = Signal(math.Derivative(math.EW.Avg(orderbook.MidPrice(book),alpha)),threshold) // defined at .output\names.sc: 1440.9
        
        /** Side function for fundamental value strategy
         */
        @python.observable()
        def FundamentalValue(/** observable fundamental value */ fv = constant(200.0),
                             /** asset in question */ book = orderbook.OfTrader()) = if orderbook.bid.Price(book)>fv then side.Sell() else if orderbook.ask.Price(book)<fv then side.Buy() else side.Nothing() // defined at .output\names.sc: 1446.9
        ...
        def MeanReversion(/** parameter |alpha| for exponentially weighted moving average */ alpha = 0.015,
                          /** asset in question */ book = orderbook.OfTrader()) = FundamentalValue(math.EW.Avg(orderbook.MidPrice(book),alpha),book) // defined at .output\names.sc: 1452.9
        
        /** Side function for a noise trading strategy
         */
        def Noise(side_distribution = math.random.uniform(0.0,1.0)) = if side_distribution>0.5 then side.Sell() else side.Buy() // defined at .output\names.sc: 1457.9
    ...
    package weight
    {
        package array
        {
            ...
            def array_ChooseTheBest() : .Optional[.List[.Float]] => .List[.Float] // defined at .output\names.sc: 1467.13
            
            /** Identity function for an array of floats
             */
            @python.curried("IdentityL")
            def array_IdentityL() : .Optional[.List[.Float]] => .List[.Float] // defined at .output\names.sc: 1473.13
        ...
        package trader
        {
            ...
            def trader_Efficiency() : .IAccount => .IFunction[.Float] // defined at .output\names.sc: 1482.13
            ...
            def trader_Score() : .IAccount => .IFunction[.Float] // defined at .output\names.sc: 1487.13
            
            /** Returns first derivative of a moving average of the trader efficiency
             */
            @python.curried("EfficiencyTrend")
            def trader_EfficiencyTrend(/** parameter alpha for the moving average */ alpha = 0.15) : .IAccount => .IFunction[.Float] // defined at .output\names.sc: 1495.13
            
            /** Unit function. Used to simulate uniform random choice of a strategy
             */
            @python.curried("Unit")
            def trader_Unit() : .IAccount => .IFunction[.Float] // defined at .output\names.sc: 1500.13
        ...
        package f
        {
            ...
            def f_Clamp0() : .Optional[.IFunction[.Float]] => .IFunction[.Float] // defined at .output\names.sc: 1509.13
            
            /** scaling function = atan(base^f(x))
             */
            @python.curried("AtanPow")
            def f_AtanPow(/** base for power function */ base = 1.002) : .Optional[.IFunction[.Float]] => .IFunction[.Float] // defined at .output\names.sc: 1514.13
            
            /** identity scaling = f(x)
             */
            @python.curried("IdentityF")
            def f_IdentityF() : .Optional[.IFunction[.Float]] => .IFunction[.Float] // defined at .output\names.sc: 1519.13
        ...
        @curried("array")
        def ChooseTheBest(array : Optional[List[Float]] = []) : List[Float] // defined at .output\names.sc: 1528.9
         ...
         */
        @curried("trader")
        def Efficiency(/** account in question */ trader : IAccount = trader.SingleProxy()) : IFunction[Float] = trader.Efficiency(trader) // defined at .output\names.sc: 1541.9
        ...
        @curried("trader")
        def Score(/** account in question */ trader : IAccount = trader.SingleProxy()) : IFunction[Float] // defined at .output\names.sc: 1550.9
        
        /** scaling function = max(0, f(x)) + 1
         */
        @curried("f")
        def Clamp0(/** function to scale */ f : Optional[IFunction[Float]] = constant()) : IFunction[Float] = math.Max(constant(0),f)+1 // defined at .output\names.sc: 1559.9
        
        /** Returns first derivative of a moving average of the trader efficiency
         */
        @curried("trader")
        def EfficiencyTrend(/** account in question */ trader : IAccount = trader.SingleProxy(),
                            /** parameter alpha for the moving average */ alpha = 0.15) : IFunction[Float] = math.Derivative(math.EW.Avg(trader.Efficiency(trader),alpha)) // defined at .output\names.sc: 1564.9
         ...
         */
        @curried("trader")
        def Unit(/** account in question */ trader : IAccount = trader.SingleProxy()) : IFunction[Float] = constant(1.0) // defined at .output\names.sc: 1572.9
        
        /** scaling function = atan(base^f(x))
         */
        @curried("f")
        def AtanPow(/** function to scale */ f : Optional[IFunction[Float]] = constant(),
                    /** base for power function */ base = 1.002) : IFunction[Float] = math.Atan(math.Pow(constant(base),f)) // defined at .output\names.sc: 1577.9
        
        /** Identity function for an array of floats
         */
        @python.intrinsic("strategy.weight._Identity_Impl")
        @curried("array")
        def IdentityL(array : Optional[List[Float]] = []) : List[Float] // defined at .output\names.sc: 1583.9
         ...
         */
        @curried("f")
        def IdentityF(f : Optional[IFunction[Float]] = constant()) : IFunction[Float] = f // defined at .output\names.sc: 1591.9
    ...
    package price
    {
        ...
        def LiquidityProvider(/** side of orders to create */ side = .side.Sell(),
                              ...
                              /** asset in question */ book = orderbook.OfTrader()) = orderbook.SafeSidePrice(orderbook.Queue(book,side),constant(initialValue))*priceDistr // defined at .output\names.sc: 1601.9
    ...
    package position
    {
        ...
        def DesiredPosition(/** observable desired position */ desiredPosition = const(),
                            /** trader in question */ trader = trader.SingleProxy()) = observable.Volume(desiredPosition-trader.Position(trader)-trader.PendingVolume(trader)) // defined at .output\names.sc: 1614.9
        ...
        def Bollinger_linear(/** alpha parameter for exponentially weighted moving everage and variance */ alpha = 0.15,
                             ...
                             /** trader in question */ trader = trader.SingleProxy()) = DesiredPosition(observable.OnEveryDt(1.0,math.EW.RelStdDev(orderbook.MidPrice(orderbook.OfTrader(trader)),alpha))*k,trader) // defined at .output\names.sc: 1619.9
        ...
        def RSI_linear(/** alpha parameter for exponentially moving averages of up movements and down movements */ alpha = 1.0/14.0,
                       ...
                       /** trader in question */ trader = trader.SingleProxy()) = DesiredPosition(observable.OnEveryDt(1.0,50.0-math.RSI(orderbook.OfTrader(trader),timeframe,alpha))*k,trader) // defined at .output\names.sc: 1625.9
    ...
    package account
    {
        package inner
        {
             ...
             */
            @python.curried("Real")
            def inner_Real() : .Optional[.ISingleAssetStrategy] => .IAccount // defined at .output\names.sc: 1638.13
            ...
            def inner_VirtualMarket() : .Optional[.ISingleAssetStrategy] => .IAccount // defined at .output\names.sc: 1644.13
        ...
        @curried("inner")
        def Real(/** strategy to track */ inner : Optional[ISingleAssetStrategy] = Noise()) : IAccount // defined at .output\names.sc: 1654.9
        ...
        @curried("inner")
        def VirtualMarket(/** strategy to track */ inner : Optional[ISingleAssetStrategy] = Noise()) : IAccount // defined at .output\names.sc: 1661.9
    ...
    def Combine(A = Noise(),
                B = Noise()) : ISingleAssetStrategy // defined at .output\names.sc: 1676.5
    ...
    def RSI_linear(/** order factory function */ orderFactory = order.signedVolume.MarketSigned(),
                   ...
                   /** observable scaling function that maps RSI deviation from 50 to the desired position */ k = const(-0.04),
                   /** lag for calculating up and down movements */ timeframe = 1.0) = Generic(orderFactory(position.RSI_linear(alpha,k,timeframe))) // defined at .output\names.sc: 1683.5
    ...
    def PairTrading(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                    ...
                    /** multiplier to obtain fair asset price from the reference asset price */ factor = 1.0) = Generic(orderFactory(side.PairTrading(bookToDependOn,factor)),eventGen) // defined at .output\names.sc: 1690.5
    ...
    def ChooseTheBest(/** original strategies that can be suspended */ strategies = [Noise()],
                      ...
                      /** function estimating is the strategy efficient or not */ performance = weight.trader.trader_EfficiencyTrend()) : ISingleAssetStrategy // defined at .output\names.sc: 1702.5
    ...
    def Signal(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
               ...
               /** threshold when the trader starts to act */ threshold = 0.7) = Generic(orderFactory(side.Signal(signal,threshold)),eventGen) // defined at .output\names.sc: 1713.5
    ...
    def LiquidityProvider(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                          ...
                          /** initial price which is taken if orderBook is empty */ initialValue = 100.0,
                          /** defines multipliers for current asset price when price of
                            *                    order to create is calculated*/ priceDistr = math.random.lognormvariate(0.0,0.1)) = Array([LiquidityProviderSide(eventGen,orderFactory,side.Sell(),initialValue,priceDistr),LiquidityProviderSide(eventGen,orderFactory,side.Buy(),initialValue,priceDistr)]) // defined at .output\names.sc: 1722.5
    ...
    def CrossingAverages(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                         ...
                         /** threshold when the trader starts to act */ threshold = 0.0) = Generic(orderFactory(side.CrossingAverages(ewma_alpha_1,ewma_alpha_2,threshold)),eventGen) // defined at .output\names.sc: 1730.5
    ...
    def Suspendable(/** wrapped strategy */ inner = Noise(),
                    /** predicate to evaluate */ predicate = true() : IFunction[Boolean]) : ISingleAssetStrategy // defined at .output\names.sc: 1741.5
    ...
    def TrendFollower(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                      ...
                      /** threshold when the trader starts to act */ threshold = 0.0) = Generic(orderFactory(side.TrendFollower(ewma_alpha,threshold)),eventGen) // defined at .output\names.sc: 1747.5
    ...
    def FundamentalValue(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                         /** order factory function*/ orderFactory = order.side.Market(),
                         /** defines fundamental value */ fundamentalValue = constant(100.0)) = Generic(orderFactory(side.FundamentalValue(fundamentalValue)),eventGen) // defined at .output\names.sc: 1759.5
    ...
    def Arbitrage() : IMultiAssetStrategy // defined at .output\names.sc: 1767.5
    ...
    def RSIbis(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
               ...
               /** strategy starts to act once RSI is out of [50-threshold, 50+threshold] */ threshold = 30.0) = Generic(orderFactory(side.Signal(50.0-math.RSI(orderbook.OfTrader(),timeframe,alpha),50.0-threshold)),eventGen) // defined at .output\names.sc: 1775.5
    ...
    def TradeIfProfitable(/** wrapped strategy */ inner = Noise(),
                          ...
                          /** given a trading account tells should it be considered as effective or not */ performance = weight.trader.trader_EfficiencyTrend()) = Suspendable(inner,performance(account(inner))>=0) // defined at .output\names.sc: 1785.5
    
    /** Creates a strategy combining an array of strategies
     */
    @python.intrinsic("strategy.combine._Array_Impl")
    def Array(/** strategies to combine */ strategies = [Noise()]) : ISingleAssetStrategy // defined at .output\names.sc: 1792.5
    ...
    def MeanReversion(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                      /** order factory function*/ orderFactory = order.side.Market(),
                      /** parameter |alpha| for exponentially weighted moving average */ ewma_alpha = 0.15) = Generic(orderFactory(side.MeanReversion(ewma_alpha)),eventGen) // defined at .output\names.sc: 1797.5
    
    /** Empty strategy doing nothing
     */
    @python.intrinsic("strategy.basic._Empty_Impl")
    def Empty() : ISingleAssetStrategy // defined at .output\names.sc: 1806.5
    ...
    def MultiArmedBandit(/** original strategies that can be suspended */ strategies = [Noise()],
                         ...
                         /** given array of strategy weights corrects them.
                           * for example it may set to 0 all weights except the maximal one */ corrector = weight.array.array_IdentityL()) : ISingleAssetStrategy // defined at .output\names.sc: 1811.5
    ...
    def Canceller(/** intervals between order cancellations */ cancellationIntervalDistr = math.random.expovariate(1.0)) : ISingleAssetStrategy // defined at .output\names.sc: 1840.5
    ...
    def LiquidityProviderSide(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                              ...
                              /** initial price which is taken if orderBook is empty */ initialValue = 100.0,
                              /** defines multipliers for current asset price when price of
                                *                    order to create is calculated*/ priceDistr = math.random.lognormvariate(0.0,0.1)) = Generic(orderFactory(side,price.LiquidityProvider(side,initialValue,priceDistr)),eventGen) // defined at .output\names.sc: 1847.5
    ...
    def Generic(/** order factory function*/ orderFactory = order.Limit(),
                /** Event source making the strategy to wake up*/ eventGen = event.Every()) : ISingleAssetStrategy // defined at .output\names.sc: 1856.5
    ...
    def Noise(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
              /** order factory function*/ orderFactory = order.side.Market()) = Generic(orderFactory(side.Noise()),eventGen) // defined at .output\names.sc: 1866.5
    ...
    def Bollinger_linear(/** order factory function */ orderFactory = order.signedVolume.MarketSigned(),
                         ...
                         /** observable scaling function that maps relative deviation to desired position */ k = const(0.5)) = Generic(orderFactory(position.Bollinger_linear(alpha,k))) // defined at .output\names.sc: 1871.5
...
package trader
{
    ...
    def Balance(trader = SingleProxy() : IAccount) : IObservable[Price] // defined at .output\names.sc: 1882.5
    
    /** Returns traders naive approximation of trader eficiency.
     *  It takes into account only the best price of the order queue
     */
    def RoughPnL(trader = SingleProxy() : IAccount) = observable.Float(Balance(trader)+orderbook.NaiveCumulativePrice(orderbook.OfTrader(trader),Position(trader))) // defined at .output\names.sc: 1887.5
    ...
    def Position(trader = SingleProxy() : IAccount) : IObservable[Volume] // defined at .output\names.sc: 1892.5
    
    /** Returns traders eficiency. Under efficiency we understand trader balance if trader position was cleared
     */
    def Efficiency(trader = SingleProxy() : IAccount) = observable.Float(Balance(trader)+orderbook.CumulativePrice(orderbook.OfTrader(trader),Position(trader))) // defined at .output\names.sc: 1899.5
    ...
    @label = "N/A"
    def SingleProxy() : ISingleAssetTrader // defined at .output\names.sc: 1903.5
    ...
    def MultiAsset(/** defines accounts for every asset to trade */ traders = [] : List[ISingleAssetTrader],
                   ...
                   /** defines what data should be gathered for the trader */ timeseries = [] : List[ITimeSerie]) : ITrader // defined at .output\names.sc: 1910.5
    
    /** Returns first derivative of a moving average of the trader efficiency
     */
    def EfficiencyTrend(trader = SingleProxy() : IAccount,
                        alpha = 0.15) = math.Derivative(math.EW.Avg(Efficiency(trader),alpha)) // defined at .output\names.sc: 1922.5
    
    /** Cumulative volume of orders sent to the market but haven't matched yet
     */
    @python.intrinsic("trader.props.PendingVolume_Impl")
    def PendingVolume(trader = SingleProxy() : IAccount) : IObservable[Volume] // defined at .output\names.sc: 1927.5
    ...
    def SingleAsset(/** order book for the asset being traded */ orderBook : IOrderBook,
                    ...
                    /** current trader balance (number of money units that it owns) */ PnL = 0.0,
                    /** defines what data should be gathered for the trader */ timeseries = [] : List[ITimeSerie]) : ISingleAssetTrader // defined at .output\names.sc: 1932.5
...
package orderbook
{
    ...
    package ask
    {
        ...
        def WeightedPrice(book = OfTrader(),
                          alpha = 0.15) = orderbook.WeightedPrice(_queue(book),alpha) // defined at .output\names.sc: 1951.9
        
        @label = "LastTradeVolume({{queue}})"
        def LastTradeVolume(book = OfTrader()) = orderbook.LastTradeVolume(_queue(book)) // defined at .output\names.sc: 1955.9
        
        @label = "{{queue}}"
        def Price(book = OfTrader()) = BestPrice(_queue(book)) // defined at .output\names.sc: 1958.9
        
        @label = "Last({{queue}})"
        def LastPrice(book = OfTrader()) = orderbook.LastPrice(_queue(book)) // defined at .output\names.sc: 1961.9
        
        def _queue = Asks
        
        @label = "LastTrade({{queue}})"
        def LastTradePrice(book = OfTrader()) = orderbook.LastTradePrice(_queue(book)) // defined at .output\names.sc: 1966.9
    ...
    @label = "N/A"
    def Proxy() : IOrderBook // defined at .output\names.sc: 1994.5
    ...
    def SafeSidePrice(queue = Asks(),
                      /** price to be used if there haven't been any trades */ defaultValue = constant(100.0)) = observable.Price(IfDefined(BestPrice(queue),IfDefined(LastPrice(queue),defaultValue))) // defined at .output\names.sc: 2002.5
    
    /** Returns moving average of trade prices weighted by their volumes
     */
    @label = "Price_{%(alpha)s}^{%(queue)s}"
    def WeightedPrice(queue = Asks(),
                      /** parameter alpha for the moving average  */ alpha = 0.15) = math.EW.Avg(LastTradePrice(queue)*LastTradeVolume(queue),alpha)/math.EW.Avg(LastTradeVolume(queue),alpha) // defined at .output\names.sc: 2009.5
    
    /** Returns tick size for the order *book*
     */
    @python.intrinsic("orderbook.props._TickSize_Impl")
    def TickSize(book = OfTrader()) : () => Price // defined at .output\names.sc: 2015.5
    
    /** MidPrice of order *book*
     */
    def MidPrice(book = OfTrader()) = observable.Price((ask.Price(book)+bid.Price(book))/2.0) // defined at .output\names.sc: 2020.5
    
    /** Returns sell side order queue for *book*
     */
    @python.intrinsic("orderbook.proxy._Asks_Impl")
    def Asks(book = OfTrader()) = Queue(book,side.Sell()) // defined at .output\names.sc: 2024.5
    ...
    def TwoWayLink(/** Forward link (normally from a trader to a market)*/ up = Link(),
                   /** Backward link (normally from a market to a trader)*/ down = Link()) : ITwoWayLink // defined at .output\names.sc: 2046.5
    
    /** Returns order queue of order *book* for trade *side*
     */
    @python.intrinsic("orderbook.proxy._Queue_Impl")
    def Queue(book = OfTrader(),
              side = side.Sell()) : IOrderQueue // defined at .output\names.sc: 2055.5
    ...
    @label = "N/A"
    def OfTrader(Trader = trader.SingleProxy() : IAccount) : IOrderBook // defined at .output\names.sc: 2061.5
    ...
    def CumulativePrice(book = OfTrader(),
                        depth = constant()) : IObservable[Price] // defined at .output\names.sc: 2069.5
    ...
    def VolumeLevels(queue = Asks(),
                     ...
                     /** number of volume levels to track */ volumeCount = 10) : IObservable[IVolumeLevels] // defined at .output\names.sc: 2081.5
    ...
    def LastPrice(queue = Asks()) : IObservable[Price] // defined at .output\names.sc: 2090.5
    ...
    def Local(tickSize = 0.01,
              _digitsToShow = 2,
              name = "-orderbook-",
              timeseries = [] : List[ITimeSerie]) : IOrderBook // defined at .output\names.sc: 2096.5
    ...
    def NaiveCumulativePrice(book = OfTrader(),
                             depth = constant()) = observable.Price(if depth<0.0 then depth*ask.Price(book) else if depth>0.0 then depth*bid.Price(book) else 0.0) // defined at .output\names.sc: 2115.5
    ...
    def Link(/** function called for each packet in order to determine
               * when it will appear at the end point*/ latency = const(0.001)) : ILink // defined at .output\names.sc: 2124.5
    
    /** Spread of order *book*
     */
    def Spread(book = OfTrader()) = observable.Price(ask.Price(book)-bid.Price(book)) // defined at .output\names.sc: 2132.5
    
    /** Returns price of the last trade at *queue*
     *  Returns None if there haven't been any trades
     */
    @python.intrinsic("orderbook.last_trade._LastTradePrice_Impl")
    def LastTradePrice(queue = Asks()) : IObservable[Price] // defined at .output\names.sc: 2136.5
...
package observable
{
    ...
    def OnEveryDt(/** time discretization step */ dt = 1.0,
                  /** function to discretize */ x = constant()) : IObservable[Float] // defined at .output\names.sc: 2147.5
    ...
    @label = "[%(x)s]"
    def Volume(x = const() : IFunction[Float]) : IObservable[Volume] // defined at .output\names.sc: 2155.5
    ...
    @label = "[%(x)s]"
    def Side(x = side.Sell() : IFunction[Side]) : IObservable[Side] // defined at .output\names.sc: 2162.5
    ...
    @python.intrinsic("observable.on_every_dt._Observable_Impl")
    @label = "[%(x)s]"
    def Price(x = const() : IFunction[Float]) : IObservable[Price] // defined at .output\names.sc: 2169.5
    ...
    def BreaksAtChanges(source = constant(1.0)) : IObservable[Float] // defined at .output\names.sc: 2176.5
    ...
    @python.intrinsic("observable.on_every_dt._Observable_Impl")
    @label = "[%(x)s]"
    def Float(x = const() : IFunction[Float]) : IObservable[Float] // defined at .output\names.sc: 2182.5
    ...
    def Quote(/** defines quotes to download */ ticker = "^GSPC",
              ...
              /** defines last day to download in form YYYY-MM-DD */ end = "2010-1-1") : IObservable[Price] // defined at .output\names.sc: 2189.5
...
package trash
{
    ...
    package in1
    {
        package in2
        {
            def S1(y = "abc") = y // defined at .output\names.sc: 2219.13
            
            def F(x = IntFunc() : IFunction[Float]) = x // defined at .output\names.sc: 2221.13
            
            def A(x = constant(),
                  y = if 3>x+2 then x else x*2) : () => types.T // defined at .output\names.sc: 2223.13
            
            def IntObs() : IObservable[Int] // defined at .output\names.sc: 2226.13
            
            def IntFunc() : IFunction[Int] // defined at .output\names.sc: 2228.13
            
            def C(x : IFunction[CandleStick],
                  p = [12,23.2,0]) = p // defined at .output\names.sc: 2230.13
            
            def S2() : Optional[String] = S1() // defined at .output\names.sc: 2233.13
            
            def O(x = IntObs() : IObservable[Float]) = x // defined at .output\names.sc: 2235.13
            
        }
        
        def A(x : () => .trash.types.T1 = .trash.A()) : () => types.U // defined at .output\names.sc: 2239.9
        
        def toInject1() : () => Int // defined at .output\names.sc: 2241.9
        
        def toInject2() : () => Int // defined at .output\names.sc: 2243.9
        
    }
    
    def A(x = in1.in2.A()) : () => types.R // defined at .output\names.sc: 2247.5
...
@category = "Basic"
@label = "C=%(x)s"
def constant(x = 1.0) = const(x) : IFunction[Float] // defined at .output\names.sc: 2251.1
...
@label = "False"
def false() : IObservable[Boolean] // defined at .output\names.sc: 2257.1
...
def null() : () => Float // defined at .output\names.sc: 2282.1
...
def TimeSerie(source = const(0.0) : IObservable[Any],
              graph = veusz.Graph(),
              _digitsToShow = 4,
              _smooth = 1) : ITimeSerie // defined at .output\names.sc: 2290.1
...
@label = "C=%(x)s"
def const(x = 1.0) : IObservable[Float] // defined at .output\names.sc: 2313.1
...
def CandleSticks(/** observable data source considered as asset price */ source = const(),
                 /** size of timeframe */ timeframe = 10.0) : IObservable[CandleStick] // defined at .output\names.sc: 2328.1
...
@label = "True"
def true() : IObservable[Boolean] // defined at .output\names.sc: 2339.1
...
def IfDefined(x = constant(),
              /** function to take values from when *x* is undefined */ elsePart = constant()) = if x<>null() then x else elsePart // defined at .output\names.sc: 2354.1
...
def volumeLevels(source : IFunction[IVolumeLevels],
                 ...
                 _smooth = 1,
                 _volumes = [30.0],
                 _isBuy = 1) : ITimeSerie // defined at .output\names.sc: 2366.1
