...
package ops
{
    ...
    @label = "-%(x)s"
    def Negate(x = constant(1.0)) : IObservable[Float] // defined at defs\ops.sc: 4.5
    
    @label = "({%(x)s}{{symbol}}{%(y)s})"
    @python.intrinsic.observable("ops._Add_Impl")
    @symbol = "+"
    def Add(x = constant(1.0),
            y = constant(1.0)) : IObservable[Float] // defined at <undefined filename>: <undefined position>
    
    @label = "({%(x)s}{{symbol}}{%(y)s})"
    @python.intrinsic.observable("ops._Less_Impl")
    @symbol = "<"
    def Less(x = constant(1.0),
             y = constant(1.0)) : IObservable[Boolean] // defined at <undefined filename>: <undefined position>
    ...
    def Condition_Float(cond = true() : IFunction[Boolean],
                        ifpart = constant(1.0),
                        elsepart = constant(1.0)) : IObservable[Float] // defined at defs\ops.sc: 28.5
    ...
    def Condition_Side(cond = true() : IFunction[Boolean],
                       ifpart = side.Sell(),
                       elsepart = side.Buy()) : IObservable[Side] // defined at defs\ops.sc: 34.5
    ...
    def Div(x = constant(1.0),
            y = constant(1.0)) : IObservable[Float] // defined at defs\ops.sc: 8.5
...
package math
{
    ...
    package Cumulative
    {
         ...
         */
        @label = "RSD{{suffix}}"
        def RelStdDev(/** observable data source */ source = const()) = (source-Avg(source))/StdDev(source) // defined at defs\math\moments.sc: 64.13
        
        /** Cumulative variance
         */
        @python.intrinsic("moments.cmv.Variance_Impl")
        @label = "\\sigma^2{{suffix}}"
        def Var(/** observable data source */ source = const()) : () => Float // defined at defs\math\moments.sc: 50.13
        
        /** Cumulative average
         */
        @python.intrinsic("moments.cma.CMA_Impl")
        @label = "Avg{{suffix}}"
        def Avg(/** observable data source */ source = const()) : () => Float // defined at defs\math\moments.sc: 43.13
        ...
        def MinEpsilon(/** observable data source */ source = constant(),
                       /** tolerance step         */ epsilon = constant(0.01)) : IObservable[Float] // defined at defs\math\minmax.sc: 26.9
        ...
        def MaxEpsilon(/** observable data source */ source = constant(),
                       /** tolerance step         */ epsilon = constant(0.01)) : IObservable[Float] // defined at defs\math\minmax.sc: 36.9
        
        /** Cumulative standard deviation
         */
        @label = "\\sqrt{\\sigma^2{{suffix}}}"
        def StdDev(/** observable data source */ source = const()) = Sqrt(Var(source)) // defined at defs\math\moments.sc: 57.13
    ...
    package macd
    {
        ...
        def MACD(/** source */ x = const(),
                 ...
                 /** short period */ fast = 12.0) = EW.Avg(x,2.0/(fast+1))-EW.Avg(x,2.0/(slow+1)) // defined at defs\math\macd.sc: 4.5
        ...
        def Signal(/** source */ x = const(),
                   ...
                   /** discretization step */ step = 1.0) = EW.Avg(observable.OnEveryDt(step,MACD(x,slow,fast)),2/(timeframe+1)) // defined at defs\math\macd.sc: 14.5
        ...
        def Histogram(/** source */ x = const(),
                      ...
                      /** short period */ fast = 12.0,
                      /** signal period */ timeframe = 9.0,
                      /** discretization step */ step = 1.0) = MACD(x,slow,fast)-Signal(x,slow,fast,timeframe,step) // defined at defs\math\macd.sc: 26.5
    ...
    package EW
    {
        ...
        def Avg(/** observable data source */ source = const(),
                /** alpha parameter */ alpha = 0.015) : IDifferentiable // defined at defs\math\moments.sc: 9.13
        ...
        def Var(/** observable data source */ source = const(),
                /** alpha parameter */ alpha = 0.015) : () => Float // defined at defs\math\moments.sc: 17.13
        
        /** Exponentially weighted moving standard deviation
         */
        @label = "\\sqrt{\\sigma^2{{suffix}}}"
        def StdDev(/** observable data source */ source = const(),
                   /** alpha parameter */ alpha = 0.015) = Sqrt(Var(source,alpha)) // defined at defs\math\moments.sc: 24.13
        
        /** Exponentially weighted moving relative standard deviation
         */
        @label = "RSD{{suffix}}"
        def RelStdDev(/** observable data source */ source = const(),
                      /** alpha parameter */ alpha = 0.015) = (source-Avg(source,alpha))/StdDev(source,alpha) // defined at defs\math\moments.sc: 31.13
    ...
    package Moving
    {
        ...
        def Min(/** observable data source */ source = constant(),
                /** sliding window size    */ timeframe = 100.0) : IObservable[Float] // defined at defs\math\minmax.sc: 6.9
        
        /** Simple moving relative standard deviation
         */
        @label = "RSD{{suffix}}"
        def RelStdDev(/** observable data source */ source = const(),
                      /** sliding window size    */ timeframe = 100.0) = (source-Avg(source,timeframe))/StdDev(source,timeframe) // defined at defs\math\moments.sc: 99.13
        ...
        @label = "\\sigma^2{{suffix}}"
        def Var(/** observable data source */ source = const(),
                /** sliding window size    */ timeframe = 100.0) = math.Max(const(0),Avg(source*source,timeframe)-Sqr(Avg(source,timeframe))) // defined at defs\math\moments.sc: 83.13
        ...
        def Max(/** observable data source */ source = constant(),
                /** sliding window size    */ timeframe = 100.0) : IObservable[Float] // defined at defs\math\minmax.sc: 14.9
        ...
        @label = "Avg{{suffix}}"
        def Avg(/** observable data source */ source = const(),
                /** sliding window size    */ timeframe = 100.0) : () => Float // defined at defs\math\moments.sc: 75.13
        
        /** Simple moving standard deviation
         */
        @label = "\\sqrt{\\sigma^2{{suffix}}}"
        def StdDev(/** observable data source */ source = const(),
                   /** sliding window size    */ timeframe = 100.0) = Sqrt(Var(source)) // defined at defs\math\moments.sc: 92.13
    ...
    @label = "min{%(x)s, %(y)s}"
    def Min(x = constant(),
            y = constant()) = if x<y then x else y // defined at <undefined filename>: <undefined position>
    ...
    @python.mathops("atan")
    def Atan(x = constant(0.0)) : () => Float // defined at <undefined filename>: <undefined position>
    ...
    @python.observable()
    @label = "{%(x)s}^2"
    def Sqr(x = constant()) = x*x // defined at <undefined filename>: <undefined position>
    ...
    @python.mathops("sqrt")
    @label = "\\sqrt{%(x)s}"
    def Sqrt(x = constant(1.0)) : () => Float // defined at <undefined filename>: <undefined position>
    ...
    @python.mathops("exp")
    @label = "e^{%(x)s}"
    def Exp(x = constant(1.0)) : () => Float // defined at <undefined filename>: <undefined position>
    ...
    @python.mathops("log")
    @label = "log(%(x)s)"
    def Log(x = constant(1.0)) : () => Float // defined at <undefined filename>: <undefined position>
    ...
    @label = "\\frac{d%(x)s}{dt}"
    def Derivative(x = math.EW.Avg() : IDifferentiable) : () => Float // defined at <undefined filename>: <undefined position>
    ...
    def Pow(base = constant(1.0),
            power = constant(1.0)) : () => Float // defined at <undefined filename>: <undefined position>
...
package order
{
    ...
    package _curried
    {
        ...
        def side_ImmediateOrCancel(/** factory for underlying orders */ proto = side.Limit()) : (() => .Side) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def volume_price_Limit(/** function defining side of orders to create */ side = side.Sell()) : (() => .Float) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
        ...
        def sidevolume_Iceberg(/** maximal size of order to send */ lotSize = constant(10.0),
                               /** underlying orders to create */ proto = side_volume.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def side_price_StopLoss(/** maximal acceptable loss factor */ maxloss = constant(0.1),
                                /** underlying orders to create */ proto = side.price.Limit()) : (() => .Side) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
        ...
        def price_Iceberg(/** maximal size of order to send */ lotSize = constant(10.0),
                          /** underlying orders to create */ proto = price.Limit()) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("ImmediateOrCancel")
        def price_ImmediateOrCancel(/** factory for underlying orders */ proto = price.Limit()) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def volume_price_WithExpiry(/** expiration period for orders */ expiry = constant(10.0),
                                    /** underlying orders to create */ proto = volume.price.Limit()) : (() => .Float) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
        ...
        def sideprice_StopLoss(/** maximal acceptable loss factor */ maxloss = constant(0.1),
                               /** underlying orders to create */ proto = side_price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def volume_price_Peg(proto = volume.price.Limit()) : (() => .Float) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
        ...
        def side_Iceberg(/** maximal size of order to send */ lotSize = constant(10.0),
                         /** underlying orders to create */ proto = side.Limit()) : (() => .Side) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def sidevolume_price_WithExpiry(/** expiration period for orders */ expiry = constant(10.0),
                                        /** underlying orders to create */ proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
        ...
        def volume_Peg(proto = volume.price.Limit()) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("ImmediateOrCancel")
        def sidevolume_ImmediateOrCancel(/** factory for underlying orders */ proto = side_volume.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def side_FixedBudget(/** function defining budget on which it may send orders at one time */ budget = constant(1000.0)) : (() => .Side) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def sideprice_Limit(/** function defining volume of orders to create */ volume = constant(1.0)) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("Peg")
        def sideprice_Peg(proto = side.price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("Peg")
        def sidevolume_Peg(proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("Peg")
        def side_Peg(proto = side.price.Limit()) : (() => .Side) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def signedVolume_LimitSigned(/** function defining price of orders to create */ price = constant(100.0)) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def side_price_ImmediateOrCancel(/** factory for underlying orders */ proto = side.price.Limit()) : (() => .Side) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("Peg")
        def price_Peg(proto = price.Limit()) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def volume_StopLoss(/** maximal acceptable loss factor */ maxloss = constant(0.1),
                            /** underlying orders to create */ proto = volume.Limit()) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.curried("price_Limit")
        def sidevolume_price_Limit() : ((() => .Side),(() => .Float)) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("ImmediateOrCancel")
        def volume_ImmediateOrCancel(/** factory for underlying orders */ proto = volume.Limit()) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def volume_FloatingPrice(/** observable defining price of orders to create */ floatingPrice = const(10.0),
                                 /** underlying orders to create */ proto = volume.price.Limit()) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def volume_Market(/** function defining side of orders to create */ side = side.Sell()) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("price_ImmediateOrCancel")
        def volume_price_ImmediateOrCancel(/** factory for underlying orders */ proto = volume.price.Limit()) : (() => .Float) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
        ...
        def side_Limit(/** function defining price of orders to create */ price = constant(100.0),
                       /** function defining volume of orders to create */ volume = constant(1.0)) : (() => .Side) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def side_FloatingPrice(/** observable defining price of orders to create */ floatingPrice = const(10.0),
                               /** underlying orders to create */ proto = side.price.Limit()) : (() => .Side) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.curried("price_Limit")
        def side_price_Limit(/** function defining volume of orders to create */ volume = constant(1.0)) : (() => .Side) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.curried("Market")
        def side_Market(/** function defining volume of orders to create */ volume = constant(1.0)) : (() => .Side) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def sidevolume_FloatingPrice(/** observable defining price of orders to create */ floatingPrice = const(10.0),
                                     /** underlying orders to create */ proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("price_ImmediateOrCancel")
        def sidevolume_price_ImmediateOrCancel(/** factory for underlying orders */ proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.curried("Market")
        def sidevolume_Market() : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
        ...
        def price_Limit(/** function defining side of orders to create */ side = side.Sell(),
                        /** function defining volume of orders to create */ volume = constant(1.0)) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.curried("Limit")
        def sidevolume_Limit(/** function defining price of orders to create */ price = constant(100.0)) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("price_Peg")
        def sidevolume_price_Peg(proto = side_volume.price.Limit()) : ((() => .Side),(() => .Float)) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
        ...
        def signedVolume_MarketSigned() : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("ImmediateOrCancel")
        def sideprice_ImmediateOrCancel(/** factory for underlying orders */ proto = side_price.Limit()) : ((() => .Side),(() => .Float)) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
         ...
         */
        @python.order.factory.on_proto("price_Peg")
        def side_price_Peg(proto = side.price.Limit()) : (() => .Side) => ((() => .Float) => .IOrderGenerator) // defined at <undefined filename>: <undefined position>
        ...
        def volume_Limit(/** function defining side of orders to create */ side = side.Sell(),
                         /** function defining price of orders to create */ price = constant(100.0)) : (() => .Float) => .IOrderGenerator // defined at <undefined filename>: <undefined position>
    ...
    def Limit(/** function defining side of orders to create */ side = side.Sell(),
              ...
              /** function defining volume of orders to create */ volume = constant(1.0)) : IOrderGenerator // defined at defs\order.sc: 23.5
    ...
    def MarketSigned(/**signed volume*/ signedVolume : () => .Float = constant(1.0)) : .IOrderGenerator // defined at <undefined filename>: <undefined position>
    ...
    def ImmediateOrCancel(/** factory for underlying orders */ proto = Limit()) : IOrderGenerator // defined at defs\order.sc: 54.5
    ...
    def Market(/** function defining side of orders to create */ side = side.Sell(),
               /** function defining volume of orders to create */ volume = constant(1.0)) : IOrderGenerator // defined at defs\order.sc: 12.5
    ...
    def StopLoss(/** maximal acceptable loss factor */ maxloss = constant(0.1),
                 /** underlying orders to create */ proto = Limit()) : IOrderGenerator // defined at defs\order.sc: 68.5
    ...
    def WithExpiry(/** expiration period for orders */ expiry = constant(10.0),
                   /** underlying orders to create */ proto = Limit()) : IOrderGenerator // defined at defs\order.sc: 82.5
    ...
    def FloatingPrice(/** observable defining price of orders to create */ floatingPrice = const(10.0),
                      /** underlying orders to create */ proto = price.Limit()) : IOrderGenerator // defined at defs\order.sc: 107.5
    ...
    def Iceberg(/** maximal size of order to send */ lotSize = constant(10.0),
                /** underlying orders to create */ proto = Limit()) : IOrderGenerator // defined at defs\order.sc: 94.5
    ...
    def FixedBudget(/** function defining side of orders to create */ side = side.Sell(),
                    /** function defining budget on which it may send orders at one time */ budget = constant(1000.0)) : IOrderGenerator // defined at defs\order.sc: 38.5
    ...
    def LimitSigned(/**signed volume*/ signedVolume : () => .Float = constant(1.0),
                    /** function defining price of orders to create */ price = constant(100.0)) : .IOrderGenerator // defined at <undefined filename>: <undefined position>
    ...
    def Peg(proto = price.Limit()) : IOrderGenerator // defined at defs\order.sc: 120.5
...
package strategy
{
    ...
    package side
    {
        ...
        def PairTrading(/** reference to order book for another asset used to evaluate fair price of our asset */ bookToDependOn = orderbook.OfTrader(),
                        ...
                        /** asset in question */ book = orderbook.OfTrader()) = observable.Side(FundamentalValue(orderbook.MidPrice(bookToDependOn)*factor,book)) // defined at defs\strategies\parts\side.sc: 82.5
        
        /** Side function for signal strategy
         */
        @python.observable()
        def Signal(/** signal to be listened to */ signal = constant(0.0),
                   /** threshold when the trader starts to act */ threshold = 0.7) = if signal>threshold then side.Buy() else if signal<0-threshold then side.Sell() else side.Nothing() // defined at defs\strategies\parts\side.sc: 11.5
        ...
        def CrossingAverages(/** parameter |alpha| for exponentially weighted moving average 1 */ alpha_1 = 0.15,
                             ...
                             /** asset in question */ book = orderbook.OfTrader()) = Signal(math.EW.Avg(orderbook.MidPrice(book),alpha_1)-math.EW.Avg(orderbook.MidPrice(book),alpha_2),threshold) // defined at defs\strategies\parts\side.sc: 37.5
        ...
        def TrendFollower(/** parameter |alpha| for exponentially weighted moving average */ alpha = 0.15,
                          /** threshold when the trader starts to act */ threshold = 0.0,
                          /** asset in question */ book = orderbook.OfTrader()) = Signal(math.Derivative(math.EW.Avg(orderbook.MidPrice(book),alpha)),threshold) // defined at defs\strategies\parts\side.sc: 24.5
        
        /** Side function for fundamental value strategy
         */
        @python.observable()
        def FundamentalValue(/** observable fundamental value */ fv = constant(200.0),
                             /** asset in question */ book = orderbook.OfTrader()) = if orderbook.bid.Price(book)>fv then side.Sell() else if orderbook.ask.Price(book)<fv then side.Buy() else side.Nothing() // defined at defs\strategies\parts\side.sc: 55.5
        ...
        def MeanReversion(/** parameter |alpha| for exponentially weighted moving average */ alpha = 0.015,
                          /** asset in question */ book = orderbook.OfTrader()) = FundamentalValue(math.EW.Avg(orderbook.MidPrice(book),alpha),book) // defined at defs\strategies\parts\side.sc: 69.5
        
        /** Side function for a noise trading strategy
         */
        def Noise(side_distribution = math.random.uniform(0.0,1.0)) = if side_distribution>0.5 then side.Sell() else side.Buy() // defined at defs\strategies\parts\side.sc: 4.5
    ...
    package weight
    {
        package array
        {
            ...
            def array_ChooseTheBest() : .Optional[.List[.Float]] => .List[.Float] // defined at <undefined filename>: <undefined position>
            
            /** Identity function for an array of floats
             */
            @python.curried("IdentityL")
            def array_IdentityL() : .Optional[.List[.Float]] => .List[.Float] // defined at <undefined filename>: <undefined position>
        ...
        package trader
        {
            ...
            def trader_Efficiency() : .IAccount => .IFunction[.Float] // defined at <undefined filename>: <undefined position>
            ...
            def trader_Score() : .IAccount => .IFunction[.Float] // defined at <undefined filename>: <undefined position>
            
            /** Returns first derivative of a moving average of the trader efficiency
             */
            @python.curried("EfficiencyTrend")
            def trader_EfficiencyTrend(/** parameter alpha for the moving average */ alpha = 0.15) : .IAccount => .IFunction[.Float] // defined at <undefined filename>: <undefined position>
            
            /** Unit function. Used to simulate uniform random choice of a strategy
             */
            @python.curried("Unit")
            def trader_Unit() : .IAccount => .IFunction[.Float] // defined at <undefined filename>: <undefined position>
        ...
        package f
        {
            ...
            def f_Clamp0() : .Optional[.IFunction[.Float]] => .IFunction[.Float] // defined at <undefined filename>: <undefined position>
            
            /** scaling function = atan(base^f(x))
             */
            @python.curried("AtanPow")
            def f_AtanPow(/** base for power function */ base = 1.002) : .Optional[.IFunction[.Float]] => .IFunction[.Float] // defined at <undefined filename>: <undefined position>
            
            /** identity scaling = f(x)
             */
            @python.curried("IdentityF")
            def f_IdentityF() : .Optional[.IFunction[.Float]] => .IFunction[.Float] // defined at <undefined filename>: <undefined position>
    ...
    package price
    {
        ...
        def LiquidityProvider(/** side of orders to create */ side = .side.Sell(),
                              ...
                              /** asset in question */ book = orderbook.OfTrader()) = orderbook.SafeSidePrice(orderbook.Queue(book,side),constant(initialValue))*priceDistr // defined at defs\strategies\parts\price.sc: 4.5
    ...
    package position
    {
        ...
        def DesiredPosition(/** observable desired position */ desiredPosition = const(),
                            /** trader in question */ trader = trader.SingleProxy()) = observable.Volume(desiredPosition-trader.Position(trader)-trader.PendingVolume(trader)) // defined at defs\strategies\parts\position.sc: 4.5
        ...
        def Bollinger_linear(/** alpha parameter for exponentially weighted moving everage and variance */ alpha = 0.15,
                             ...
                             /** trader in question */ trader = trader.SingleProxy()) = DesiredPosition(observable.OnEveryDt(1.0,math.EW.RelStdDev(orderbook.MidPrice(orderbook.OfTrader(trader)),alpha))*k,trader) // defined at defs\strategies\parts\position.sc: 15.5
        ...
        def RSI_linear(/** alpha parameter for exponentially moving averages of up movements and down movements */ alpha = 1.0/14.0,
                       ...
                       /** trader in question */ trader = trader.SingleProxy()) = DesiredPosition(observable.OnEveryDt(1.0,50.0-math.RSI(orderbook.OfTrader(trader),timeframe,alpha))*k,trader) // defined at defs\strategies\parts\position.sc: 34.5
    ...
    package account
    {
        package inner
        {
             ...
             */
            @python.curried("Real")
            def inner_Real() : .Optional[.ISingleAssetStrategy] => .IAccount // defined at <undefined filename>: <undefined position>
            ...
            def inner_VirtualMarket() : .Optional[.ISingleAssetStrategy] => .IAccount // defined at <undefined filename>: <undefined position>
    ...
    def Combine(A = Noise(),
                B = Noise()) : ISingleAssetStrategy // defined at defs\strategies\common.sc: 14.5
    ...
    def RSI_linear(/** order factory function */ orderFactory = order.signedVolume.MarketSigned(),
                   ...
                   /** observable scaling function that maps RSI deviation from 50 to the desired position */ k = const(-0.04),
                   /** lag for calculating up and down movements */ timeframe = 1.0) = Generic(orderFactory(position.RSI_linear(alpha,k,timeframe))) // defined at defs\strategies\position.sc: 3.5
    ...
    def PairTrading(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                    ...
                    /** multiplier to obtain fair asset price from the reference asset price */ factor = 1.0) = Generic(orderFactory(side.PairTrading(bookToDependOn,factor)),eventGen) // defined at defs\strategies\side.sc: 115.5
    ...
    def ChooseTheBest(/** original strategies that can be suspended */ strategies = [Noise()],
                      ...
                      /** function estimating is the strategy efficient or not */ performance = weight.trader.trader_EfficiencyTrend()) : ISingleAssetStrategy // defined at defs\strategies\adaptive.sc: 49.5
    ...
    def Signal(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
               ...
               /** threshold when the trader starts to act */ threshold = 0.7) = Generic(orderFactory(side.Signal(signal,threshold)),eventGen) // defined at defs\strategies\side.sc: 16.5
    ...
    def LiquidityProvider(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                          ...
                          /** initial price which is taken if orderBook is empty */ initialValue = 100.0,
                          /** defines multipliers for current asset price when price of
                            *                    order to create is calculated*/ priceDistr = math.random.lognormvariate(0.0,0.1)) = Array([LiquidityProviderSide(eventGen,orderFactory,side.Sell(),initialValue,priceDistr),LiquidityProviderSide(eventGen,orderFactory,side.Buy(),initialValue,priceDistr)]) // defined at defs\strategies\sideprice.sc: 28.5
    ...
    def CrossingAverages(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                         ...
                         /** threshold when the trader starts to act */ threshold = 0.0) = Generic(orderFactory(side.CrossingAverages(ewma_alpha_1,ewma_alpha_2,threshold)),eventGen) // defined at defs\strategies\side.sc: 57.5
    ...
    def Suspendable(/** wrapped strategy */ inner = Noise(),
                    /** predicate to evaluate */ predicate = true() : IFunction[Boolean]) : ISingleAssetStrategy // defined at defs\strategies\adaptive.sc: 3.5
    ...
    def TrendFollower(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                      ...
                      /** threshold when the trader starts to act */ threshold = 0.0) = Generic(orderFactory(side.TrendFollower(ewma_alpha,threshold)),eventGen) // defined at defs\strategies\side.sc: 35.5
    ...
    def FundamentalValue(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                         /** order factory function*/ orderFactory = order.side.Market(),
                         /** defines fundamental value */ fundamentalValue = constant(100.0)) = Generic(orderFactory(side.FundamentalValue(fundamentalValue)),eventGen) // defined at defs\strategies\side.sc: 79.5
    ...
    def Arbitrage() : IMultiAssetStrategy // defined at defs\strategies\common.sc: 44.5
    ...
    def RSIbis(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
               ...
               /** strategy starts to act once RSI is out of [50-threshold, 50+threshold] */ threshold = 30.0) = Generic(orderFactory(side.Signal(50.0-math.RSI(orderbook.OfTrader(),timeframe,alpha),50.0-threshold)),eventGen) // defined at defs\strategies\side.sc: 137.5
    ...
    def TradeIfProfitable(/** wrapped strategy */ inner = Noise(),
                          ...
                          /** given a trading account tells should it be considered as effective or not */ performance = weight.trader.trader_EfficiencyTrend()) = Suspendable(inner,performance(account(inner))>=0) // defined at defs\strategies\adaptive.sc: 13.5
    
    /** Creates a strategy combining an array of strategies
     */
    @python.intrinsic("strategy.combine._Array_Impl")
    def Array(/** strategies to combine */ strategies = [Noise()]) : ISingleAssetStrategy // defined at defs\strategies\common.sc: 22.5
    ...
    def MeanReversion(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                      /** order factory function*/ orderFactory = order.side.Market(),
                      /** parameter |alpha| for exponentially weighted moving average */ ewma_alpha = 0.15) = Generic(orderFactory(side.MeanReversion(ewma_alpha)),eventGen) // defined at defs\strategies\side.sc: 97.5
    
    /** Empty strategy doing nothing
     */
    @python.intrinsic("strategy.basic._Empty_Impl")
    def Empty() : ISingleAssetStrategy // defined at defs\strategies\common.sc: 38.5
    ...
    def MultiArmedBandit(/** original strategies that can be suspended */ strategies = [Noise()],
                         ...
                         /** given array of strategy weights corrects them.
                           * for example it may set to 0 all weights except the maximal one */ corrector = weight.array.array_IdentityL()) : ISingleAssetStrategy // defined at defs\strategies\adaptive.sc: 27.5
    ...
    def Canceller(/** intervals between order cancellations */ cancellationIntervalDistr = math.random.expovariate(1.0)) : ISingleAssetStrategy // defined at defs\strategies\common.sc: 29.5
    ...
    def LiquidityProviderSide(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
                              ...
                              /** initial price which is taken if orderBook is empty */ initialValue = 100.0,
                              /** defines multipliers for current asset price when price of
                                *                    order to create is calculated*/ priceDistr = math.random.lognormvariate(0.0,0.1)) = Generic(orderFactory(side,price.LiquidityProvider(side,initialValue,priceDistr)),eventGen) // defined at defs\strategies\sideprice.sc: 3.5
    ...
    def Noise(/** Event source making the strategy to wake up*/ eventGen = event.Every(math.random.expovariate(1.0)),
              /** order factory function*/ orderFactory = order.side.Market()) = Generic(orderFactory(side.Noise()),eventGen) // defined at defs\strategies\side.sc: 3.5
    ...
    def Bollinger_linear(/** order factory function */ orderFactory = order.signedVolume.MarketSigned(),
                         ...
                         /** observable scaling function that maps relative deviation to desired position */ k = const(0.5)) = Generic(orderFactory(position.Bollinger_linear(alpha,k))) // defined at defs\strategies\position.sc: 19.5
...
package trader
{
    ...
    def Balance(trader = SingleProxy() : IAccount) : IObservable[Price] // defined at defs\trader\properties.sc: 12.5
    
    /** Returns traders naive approximation of trader eficiency.
     *  It takes into account only the best price of the order queue
     */
    def RoughPnL(trader = SingleProxy() : IAccount) = observable.Float(Balance(trader)+orderbook.NaiveCumulativePrice(orderbook.OfTrader(trader),Position(trader))) // defined at defs\trader\properties.sc: 36.5
    ...
    def Position(trader = SingleProxy() : IAccount) : IObservable[Volume] // defined at defs\trader\properties.sc: 4.5
    
    /** Returns traders eficiency. Under efficiency we understand trader balance if trader position was cleared
     */
    def Efficiency(trader = SingleProxy() : IAccount) = observable.Float(Balance(trader)+orderbook.CumulativePrice(orderbook.OfTrader(trader),Position(trader))) // defined at defs\trader\properties.sc: 24.5
    ...
    def EfficiencyTrend(trader = SingleProxy() : IAccount,
                        alpha = 0.15) = math.Derivative(math.EW.Avg(Efficiency(trader),alpha)) // defined at defs\trader\properties.sc: 49.5
    
    /** Cumulative volume of orders sent to the market but haven't matched yet
     */
    @python.intrinsic("trader.props.PendingVolume_Impl")
    def PendingVolume(trader = SingleProxy() : IAccount) : IObservable[Volume] // defined at defs\trader\properties.sc: 18.5
...
package orderbook
{
    ...
    package ask
    {
        ...
        def WeightedPrice(book = OfTrader(),
                          alpha = 0.15) = orderbook.WeightedPrice(_queue(book),alpha) // defined at defs\orderbook\properties.sc: 152.9
        
        @label = "LastTradeVolume({{queue}})"
        def LastTradeVolume(book = OfTrader()) = orderbook.LastTradeVolume(_queue(book)) // defined at defs\orderbook\properties.sc: 149.9
        
        @label = "{{queue}}"
        def Price(book = OfTrader()) = BestPrice(_queue(book)) // defined at defs\orderbook\properties.sc: 140.9
        
        @label = "Last({{queue}})"
        def LastPrice(book = OfTrader()) = orderbook.LastPrice(_queue(book)) // defined at defs\orderbook\properties.sc: 143.9
        
        def _queue = Asks
        
        @label = "LastTrade({{queue}})"
        def LastTradePrice(book = OfTrader()) = orderbook.LastTradePrice(_queue(book)) // defined at defs\orderbook\properties.sc: 146.9
    ...
    @label = "N/A"
    def Proxy() : IOrderBook // defined at defs\orderbook\classes.sc: 56.5
    ...
    def SafeSidePrice(queue = Asks(),
                      /** price to be used if there haven't been any trades */ defaultValue = constant(100.0)) = observable.Price(IfDefined(BestPrice(queue),IfDefined(LastPrice(queue),defaultValue))) // defined at defs\orderbook\properties.sc: 50.5
    
    /** Returns moving average of trade prices weighted by their volumes
     */
    @label = "Price_{%(alpha)s}^{%(queue)s}"
    def WeightedPrice(queue = Asks(),
                      /** parameter alpha for the moving average  */ alpha = 0.15) = math.EW.Avg(LastTradePrice(queue)*LastTradeVolume(queue),alpha)/math.EW.Avg(LastTradeVolume(queue),alpha) // defined at defs\orderbook\properties.sc: 63.5
    
    /** Returns tick size for the order *book*
     */
    @python.intrinsic("orderbook.props._TickSize_Impl")
    def TickSize(book = OfTrader()) : () => Price // defined at defs\orderbook\properties.sc: 74.5
    
    /** MidPrice of order *book*
     */
    def MidPrice(book = OfTrader()) = observable.Price((ask.Price(book)+bid.Price(book))/2.0) // defined at defs\orderbook\properties.sc: 88.5
    
    /** Returns sell side order queue for *book*
     */
    @python.intrinsic("orderbook.proxy._Asks_Impl")
    def Asks(book = OfTrader()) = Queue(book,side.Sell()) // defined at defs\orderbook\properties.sc: 10.5
    ...
    def LastTradeVolume(queue = Asks()) : IObservable[Volume] // defined at defs\orderbook\properties.sc: 43.5
    
    /** Returns buy side order queue for *book*
     */
    @python.intrinsic("orderbook.proxy._Bids_Impl")
    def Bids(book = OfTrader()) = Queue(book,side.Buy()) // defined at defs\orderbook\properties.sc: 16.5
    
    /** Returns best order price of *queue*
     *  Returns None is *queue* is empty
     */
    @python.intrinsic("orderbook.props._BestPrice_Impl")
    def BestPrice(queue = Asks()) : IObservable[Price] // defined at defs\orderbook\properties.sc: 22.5
    ...
    def TwoWayLink(/** Forward link (normally from a trader to a market)*/ up = Link(),
                   /** Backward link (normally from a market to a trader)*/ down = Link()) : ITwoWayLink // defined at defs\orderbook\classes.sc: 35.5
    
    /** Returns order queue of order *book* for trade *side*
     */
    @python.intrinsic("orderbook.proxy._Queue_Impl")
    def Queue(book = OfTrader(),
              side = side.Sell()) : IOrderQueue // defined at defs\orderbook\properties.sc: 4.5
    ...
    @label = "N/A"
    def OfTrader(Trader = trader.SingleProxy() : IAccount) : IOrderBook // defined at defs\orderbook\classes.sc: 47.5
    ...
    def CumulativePrice(book = OfTrader(),
                        depth = constant()) : IObservable[Price] // defined at defs\orderbook\properties.sc: 96.5
    ...
    def VolumeLevels(queue = Asks(),
                     ...
                     /** number of volume levels to track */ volumeCount = 10) : IObservable[IVolumeLevels] // defined at defs\orderbook\properties.sc: 124.5
    ...
    def LastPrice(queue = Asks()) : IObservable[Price] // defined at defs\orderbook\properties.sc: 29.5
    ...
    def Local(tickSize = 0.01,
              _digitsToShow = 2,
              name = "-orderbook-",
              timeseries = [] : List[ITimeSerie]) : IOrderBook // defined at defs\orderbook\classes.sc: 4.5
    ...
    def Remote(orderbook = Local(),
               link = TwoWayLink(),
               timeseries = [] : List[ITimeSerie]) : IOrderBook // defined at defs\orderbook\classes.sc: 15.5
    ...
    def NaiveCumulativePrice(book = OfTrader(),
                             depth = constant()) = observable.Price(if depth<0.0 then depth*ask.Price(book) else if depth>0.0 then depth*bid.Price(book) else 0.0) // defined at defs\orderbook\properties.sc: 109.5
    ...
    def Link(/** function called for each packet in order to determine
               * when it will appear at the end point*/ latency = const(0.001)) : ILink // defined at defs\orderbook\classes.sc: 25.5
    
    /** Spread of order *book*
     */
    def Spread(book = OfTrader()) = observable.Price(ask.Price(book)-bid.Price(book)) // defined at defs\orderbook\properties.sc: 80.5
    
    /** Returns price of the last trade at *queue*
     *  Returns None if there haven't been any trades
     */
    @python.intrinsic("orderbook.last_trade._LastTradePrice_Impl")
    def LastTradePrice(queue = Asks()) : IObservable[Price] // defined at defs\orderbook\properties.sc: 36.5
...
package trash
{
    ...
    package in1
    {
        package in2
        {
            def S1(y = "abc") = y // defined at defs\trash.sc: 34.1
            
            def F(x = IntFunc() : IFunction[Float]) = x // defined at defs\trash.sc: 42.1
            
            def A(x = constant(),
                  y = if 3>x+2 then x else x*2) : () => types.T // defined at defs\trash.sc: 32.1
            
            def IntObs() : IObservable[Int] // defined at defs\trash.sc: 44.1
            
            def IntFunc() : IFunction[Int] // defined at defs\trash.sc: 40.1
            
            def C(x : IFunction[CandleStick],
                  p = [12,23.2,0]) = p // defined at defs\trash.sc: 38.1
            
            def S2() : Optional[String] = S1() // defined at defs\trash.sc: 36.1
            
            def O(x = IntObs() : IObservable[Float]) = x // defined at defs\trash.sc: 46.1
            
        }
        
        def A(x : () => .trash.types.T1 = .trash.A()) : () => types.U // defined at defs\trash.sc: 29.1
        
        def toInject1() : () => Int // defined at defs\trash.sc: 23.27
        
        def toInject2() : () => Int // defined at defs\trash.sc: 24.27
        
    }
    
    def A(x = in1.in2.A()) : () => types.R // defined at defs\trash.sc: 26.1
...
@category = "Basic"
@label = "C=%(x)s"
def constant(x = 1.0) = const(x) : IFunction[Float] // defined at <undefined filename>: <undefined position>
...
@label = "False"
def false() : IObservable[Boolean] // defined at <undefined filename>: <undefined position>
...
def null() : () => Float // defined at <undefined filename>: <undefined position>
...
def TimeSerie(source = const(0.0) : IObservable[Any],
              graph = veusz.Graph(),
              _digitsToShow = 4,
              _smooth = 1) : ITimeSerie // defined at <undefined filename>: <undefined position>
...
@label = "C=%(x)s"
def const(x = 1.0) : IObservable[Float] // defined at <undefined filename>: <undefined position>
...
def CandleSticks(/** observable data source considered as asset price */ source = const(),
                 /** size of timeframe */ timeframe = 10.0) : IObservable[CandleStick] // defined at <undefined filename>: <undefined position>
...
@label = "True"
def true() : IObservable[Boolean] // defined at <undefined filename>: <undefined position>
...
def IfDefined(x = constant(),
              /** function to take values from when *x* is undefined */ elsePart = constant()) = if x<>null() then x else elsePart // defined at <undefined filename>: <undefined position>
...
def volumeLevels(source : IFunction[IVolumeLevels],
                 ...
                 _smooth = 1,
                 _volumes = [30.0],
                 _isBuy = 1) : ITimeSerie // defined at <undefined filename>: <undefined position>
