...
class RandomWalk(_RandomWalk_Impl):""" 
    """ 
    def __init__(self, initialValue = None, deltaDistr = None, intervalDistr = None, name = None):from marketsim.gen._out.math.random._normalvariate import normalvariate as _math_random_normalvariate
        ...
        if isinstance(initialValue, types.IEvent):event.subscribe(self.initialValue, self.fire, self)
        if isinstance(deltaDistr, types.IEvent):event.subscribe(self.deltaDistr, self.fire, self)
        if isinstance(intervalDistr, types.IEvent):event.subscribe(self.intervalDistr, self.fire, self)
        if isinstance(name, types.IEvent):event.subscribe(self.name, self.fire, self)
