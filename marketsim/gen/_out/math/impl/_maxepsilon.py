...
class MaxEpsilon_ICumulativeIObservableFloat(Observablefloat):
    """ 
    """ 
    ...
    def __repr__(self):
        return "MaxEpsilon(%(x)s, %(epsilon)s)" % self.__dict__
...
from marketsim import context
@registry.expose(["Statistics", "MaxEpsilon"])
class MaxEpsilon_ICumulativeFloat(Observablefloat):
    """ 
    """ 
    def __init__(self, x = None, epsilon = None):
        from marketsim import deref_opt
...
def MaxEpsilon(x = None,epsilon = None): 
    ...
    if x is None or rtti.can_be_casted(x, ICumulative):
        if epsilon is None or rtti.can_be_casted(epsilon, IObservablefloat):
            return MaxEpsilon_ICumulativeIObservableFloat(x,epsilon)
    if x is None or rtti.can_be_casted(x, ICumulative):
        if epsilon is None or rtti.can_be_casted(epsilon, IFunctionfloat):
            return MaxEpsilon_ICumulativeFloat(x,epsilon)
