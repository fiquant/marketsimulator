...
class Histogram(Function[float]):""" 
    ...
    def __repr__(self):return "Histogram^{%(timeframe)s}_{%(step)s}(MACD_{%(fast)s}^{%(slow)s}(%(x)s))" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _math_macd_MACD(self.x,self.slow,self.fast)-_math_macd_Signal(self.x,self.slow,self.fast,self.timeframe,self.step)
