...
class MACD(Function[float]):""" 
    """ 
    def __init__(self, x = None, slow = None, fast = None):from marketsim.gen._out._const import const as _const
    ...
    def __repr__(self):return "MACD_{%(fast)s}^{%(slow)s}(%(x)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _math_EW_Avg(self.x,2.0/(self.fast+1))-_math_EW_Avg(self.x,2.0/(self.slow+1))
