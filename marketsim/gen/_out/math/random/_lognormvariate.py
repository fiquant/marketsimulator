...
class lognormvariate(Function[float]):""" 
    ...
    """ 
    def __init__(self, Mu = None, Sigma = None):from marketsim import rtti
    ...
    def __repr__(self):return "lognormvariate(%(Mu)s, %(Sigma)s)" % self.__dict__
    
    def __call__(self, *args, **kwargs):import random
        return random.lognormvariate(self.Mu, self.Sigma)
    
    def _casts_to(self, dst):return lognormvariate._types[0]._casts_to(dst)
