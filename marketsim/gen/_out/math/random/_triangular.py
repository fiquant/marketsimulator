...
class triangular(Function[float]):""" 
    ...
    """ 
    def __init__(self, Low = None, High = None, Mode = None):from marketsim import rtti
    ...
    _properties = {'Low' : float,
        'High' : float,
        'Mode' : float
    }
    def __repr__(self):return "triangular(%(Low)s, %(High)s, %(Mode)s)" % self.__dict__
    
    def __call__(self, *args, **kwargs):import random
        return random.triangular(self.Low, self.High, self.Mode)
    
    def _casts_to(self, dst):return triangular._types[0]._casts_to(dst)
