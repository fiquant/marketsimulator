...
class vonmisesvariate(Function[float]):""" 
    """ 
    def __init__(self, Mu = None, Kappa = None):from marketsim import rtti
    ...
    _properties = {'Mu' : float,
        'Kappa' : float
    }
    def __repr__(self):return "vonmisesvariate(%(Mu)s, %(Kappa)s)" % self.__dict__
    
    def __call__(self, *args, **kwargs):import random
        return random.vonmisesvariate(self.Mu, self.Kappa)
    
    def _casts_to(self, dst):return vonmisesvariate._types[0]._casts_to(dst)
