...
class StdDev(Function[float]):""" 
    """ 
    def __init__(self, source = None, alpha = None):from marketsim.gen._out._const import const as _const
    ...
    def __repr__(self):return "\\sqrt{\\sigma^2_{\\alpha=%(alpha)s}(%(source)s)}" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _math_Sqrt(_math_EW_Var(self.source,self.alpha))
