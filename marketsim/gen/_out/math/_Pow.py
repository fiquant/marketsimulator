...
class Pow(Observable[float]):""" 
    ...
    """ 
    def __init__(self, base = None, power = None):from marketsim.ops._all import Observable
        ...
        if isinstance(base, types.IEvent):event.subscribe(self.base, self.fire, self)
        self.power = power if power is not None else _constant(1.0)
        if isinstance(power, types.IEvent):event.subscribe(self.power, self.fire, self)
        rtti.check_fields(self)
    
    @property
    def label(self):return repr(self)
    
    _properties = {'base' : IFunction[float],
        'power' : IFunction[float]
    }
    def __repr__(self):return "%(base)s^{%(power)s}" % self.__dict__
    
    def __call__(self, *args, **kwargs):import math
        base = self.base()
