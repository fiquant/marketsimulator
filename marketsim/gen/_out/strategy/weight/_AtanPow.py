...
class AtanPow(Function[float]):""" 
    """ 
    def __init__(self, f = None, base = None):from marketsim.gen._out._constant import constant as _constant
    ...
    _properties = {'f' : IFunction[float],
        'base' : float
    }
    def __repr__(self):return "AtanPow(%(f)s, %(base)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _math_Atan(_math_Pow(_constant(self.base),self.f))
