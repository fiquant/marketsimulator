...
class LiquidityProvider(Observable[float]):""" 
    """ 
    def __init__(self, side = None, initialValue = None, priceDistr = None, book = None):from marketsim import float
        from marketsim.ops._all import Observable
    ...
    def __repr__(self):return "LiquidityProvider(%(side)s, %(initialValue)s, %(priceDistr)s, %(book)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):from marketsim.gen._out.orderbook._SafeSidePrice import SafeSidePrice as _orderbook_SafeSidePrice
