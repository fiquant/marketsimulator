...
class LiquidityProvider(ISingleAssetStrategy):""" 
    """ 
    def __init__(self, eventGen = None, orderFactory = None, initialValue = None, priceDistr = None):from marketsim.gen._out.event._Every import Every as _event_Every
    ...
    def __repr__(self):return "LiquidityProvider(%(eventGen)s, %(orderFactory)s, %(initialValue)s, %(priceDistr)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _strategy_Array([_strategy_LiquidityProviderSide(self.eventGen,self.orderFactory,_side_Sell(),self.initialValue,self.priceDistr),_strategy_LiquidityProviderSide(self.eventGen,self.orderFactory,_side_Buy(),self.initialValue,self.priceDistr)])
