...
class FundamentalValue(ISingleAssetStrategy):"""  (*fundamental value*) and if the current asset price is lower than the fundamental value
     it starts to buy the asset and if the price is higher it starts to sell the asset.
    """ 
    def __init__(self, eventGen = None, orderFactory = None, fundamentalValue = None):from marketsim.gen._out.event._Every import Every as _event_Every
    ...
    def __repr__(self):return "FundamentalValue(%(eventGen)s, %(orderFactory)s, %(fundamentalValue)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _strategy_Generic(self.orderFactory(_strategy_side_FundamentalValue(self.fundamentalValue)),self.eventGen)
