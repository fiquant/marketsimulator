...
@registry.expose(["-", "BookToDependOn"])
class BookToDependOn_strategysidePairTrading(object):
    """ 
    """ 
    ...
    def __repr__(self):
        return "BookToDependOn(%(x)s)" % dict([ (name, getattr(self, name)) for name in self._properties.iterkeys() ])
    ...
    def dereference(self):
        return self.x.bookToDependOn
    
def BookToDependOn(x = None): 
    from marketsim.gen._out.strategy.side._pairtrading import PairTrading
    from marketsim import rtti
    if x is None or rtti.can_be_casted(x, PairTrading):
        return BookToDependOn_strategysidePairTrading(x)
    raise Exception('Cannot find suitable overload for BookToDependOn('+str(x) +':'+ str(type(x))+')')
