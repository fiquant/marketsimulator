...
class PairTrading(Observable[Side]):""" 
    """ 
    def __init__(self, bookToDependOn = None, factor = None, book = None):from marketsim import Side
        from marketsim.ops._all import Observable
    ...
    _properties = {'bookToDependOn' : IOrderBook,
        'factor' : float,
        'book' : IOrderBook
    }
    def __repr__(self):return "PairTrading(%(bookToDependOn)s, %(factor)s, %(book)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):from marketsim.gen._out.observable._Side import Side as _observable_Side
