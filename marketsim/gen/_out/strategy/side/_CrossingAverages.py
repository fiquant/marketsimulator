...
class CrossingAverages(Function[Side]):""" 
    """ 
    def __init__(self, alpha_1 = None, alpha_2 = None, threshold = None, book = None):from marketsim.gen._out.orderbook._OfTrader import OfTrader as _orderbook_OfTrader
    ...
    _properties = {'alpha_1' : float,
        'alpha_2' : float,
        'threshold' : float,
        'book' : IOrderBook
    }
    def __repr__(self):return "CrossingAverages(%(alpha_1)s, %(alpha_2)s, %(threshold)s, %(book)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _strategy_side_Signal(_math_EW_Avg(_orderbook_MidPrice(self.book),self.alpha_1)-_math_EW_Avg(_orderbook_MidPrice(self.book),self.alpha_2),self.threshold)
