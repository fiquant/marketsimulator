...
class TradeIfProfitable(ISingleAssetStrategy):""" 
    """ 
    def __init__(self, inner = None, account = None, performance = None):from marketsim.gen._out.strategy._Noise import Noise as _strategy_Noise
    ...
    _properties = {'inner' : ISingleAssetStrategy,
    ...
    def __repr__(self):return "TradeIfProfitable(%(inner)s, %(account)s, %(performance)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _strategy_Suspendable(self.inner,self.performance(self.account(self.inner))>=0)
