...
class RSIbis(ISingleAssetStrategy):"""   and starts to buy when RSI is greater than 50 + *threshold*
      and sells when RSI is less than 50 - *thresold*
    """ 
    def __init__(self, eventGen = None, orderFactory = None, alpha = None, timeframe = None, threshold = None):from marketsim.gen._out.event._Every import Every as _event_Every
    ...
    def __repr__(self):return "RSIbis(%(eventGen)s, %(orderFactory)s, %(alpha)s, %(timeframe)s, %(threshold)s)" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _strategy_Generic(self.orderFactory(_strategy_side_Signal(_const(50.0)-_math_RSI(_orderbook_OfTrader(),self.timeframe,self.alpha),50.0-self.threshold)),self.eventGen)
