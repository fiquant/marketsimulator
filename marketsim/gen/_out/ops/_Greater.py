...
from marketsim import float
@registry.expose(["Ops", "Greater"])
class Greater_IFunctionFloatIObservableFloat(Observable[bool],_Greater_Impl):
    """ 
    """ 
    def __init__(self, x = None, y = None):
        from marketsim import bool
...
from marketsim import float
@registry.expose(["Ops", "Greater"])
class Greater_IObservableFloatIFunctionFloat(Observable[bool],_Greater_Impl):
    """ 
    """ 
    def __init__(self, x = None, y = None):
        from marketsim import bool
...
from marketsim import float
@registry.expose(["Ops", "Greater"])
class Greater_IFunctionFloatIFunctionFloat(Observable[bool],_Greater_Impl):
    """ 
    """ 
    def __init__(self, x = None, y = None):
        from marketsim import bool
...
def Greater(x = None,y = None): 
    ...
    if x is None or rtti.can_be_casted(x, IFunction[float]):
        if y is None or rtti.can_be_casted(y, IObservable[float]):
            return Greater_IFunctionFloatIObservableFloat(x,y)
    if x is None or rtti.can_be_casted(x, IObservable[float]):
        if y is None or rtti.can_be_casted(y, IFunction[float]):
            return Greater_IObservableFloatIFunctionFloat(x,y)
    if x is None or rtti.can_be_casted(x, IFunction[float]):
        if y is None or rtti.can_be_casted(y, IFunction[float]):
            return Greater_IFunctionFloatIFunctionFloat(x,y)
    raise Exception('Cannot find suitable overload for Greater('+str(x)+','+str(y)+')')
