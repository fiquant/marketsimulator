...
from marketsim import float
@registry.expose(["Ops", "LessEqual"])
class LessEqual_IFunctionFloatIObservableFloat(Observable[bool],_LessEqual_Impl):
    """ 
    """ 
    def __init__(self, x = None, y = None):
        from marketsim import bool
...
from marketsim import float
@registry.expose(["Ops", "LessEqual"])
class LessEqual_IObservableFloatIFunctionFloat(Observable[bool],_LessEqual_Impl):
    """ 
    """ 
    def __init__(self, x = None, y = None):
        from marketsim import bool
...
def LessEqual(x = None,y = None): 
    ...
    if x is None or rtti.can_be_casted(x, IFunction[float]):
        if y is None or rtti.can_be_casted(y, IObservable[float]):
            return LessEqual_IFunctionFloatIObservableFloat(x,y)
    if x is None or rtti.can_be_casted(x, IObservable[float]):
        if y is None or rtti.can_be_casted(y, IFunction[float]):
            return LessEqual_IObservableFloatIFunctionFloat(x,y)
    if x is None or rtti.can_be_casted(x, IFunction[float]):
        if y is None or rtti.can_be_casted(y, IFunction[float]):
            return LessEqual_IFunctionFloatIFunctionFloat(x,y)
    raise Exception('Cannot find suitable overload for LessEqual('+str(x)+','+str(y)+')')
