...
class WeightedPrice(Function[float]):""" 
    """ 
    def __init__(self, queue = None, alpha = None):from marketsim.gen._out.orderbook._Asks import Asks as _orderbook_Asks
    ...
    def __repr__(self):return "Price_{%(alpha)s}^{%(queue)s}" % self.__dict__
    
    _internals = ['impl']
    def getImpl(self):return _math_EW_Avg(_orderbook_LastTradePrice(self.queue)*_orderbook_LastTradeVolume(self.queue),self.alpha)/_math_EW_Avg(_orderbook_LastTradeVolume(self.queue),self.alpha)
