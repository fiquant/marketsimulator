...
class Link(_Link_Impl):"""  (normally between a trader and a market).
     Ensures that sending packets via a link preserves their order.
    """ 
    def __init__(self, latency = None):from marketsim.gen._out._const import const as _const
    ...
    _properties = {'latency' : IObservable[float]
    }
    def __repr__(self):return "Link(%(latency)s)" % self.__dict__
