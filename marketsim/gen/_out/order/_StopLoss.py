...
class StopLoss(IOrderGenerator, Observable[Order]):""" 
    ...
    """ 
    def __init__(self, maxloss = None, proto = None):from marketsim import Order
        from marketsim.ops._all import Observable
        ...
        if isinstance(maxloss, types.IEvent):event.subscribe(self.maxloss, self.fire, self)
    ...
    _properties = {'maxloss' : IFunction[float],
        'proto' : IOrderGenerator
    }
    def __repr__(self):return "StopLoss(%(maxloss)s, %(proto)s)" % self.__dict__
    
    def __call__(self, *args, **kwargs):from marketsim.gen._intrinsic.order.meta.stoploss import Order_Impl
