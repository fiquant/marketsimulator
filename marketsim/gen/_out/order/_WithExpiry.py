...
class WithExpiry(IOrderGenerator, Observable[Order]):""" 
    ...
    """ 
    def __init__(self, expiry = None, proto = None):from marketsim import Order
        from marketsim.ops._all import Observable
        ...
        if isinstance(expiry, types.IEvent):event.subscribe(self.expiry, self.fire, self)
    ...
    def __repr__(self):return "WithExpiry(%(expiry)s, %(proto)s)" % self.__dict__
    
    def __call__(self, *args, **kwargs):from marketsim.gen._intrinsic.order.meta.with_expiry import Order_Impl
        expiry = self.expiry()
