...
class Iceberg(IOrderGenerator, Observable[Order]):""" 
    ...
    """ 
    def __init__(self, lotSize = None, proto = None):from marketsim import Order
        from marketsim.ops._all import Observable
        ...
        if isinstance(lotSize, types.IEvent):event.subscribe(self.lotSize, self.fire, self)
    ...
    def __repr__(self):return "Iceberg(%(lotSize)s, %(proto)s)" % self.__dict__
    
    def __call__(self, *args, **kwargs):from marketsim.gen._intrinsic.order.meta.iceberg import Order_Impl
