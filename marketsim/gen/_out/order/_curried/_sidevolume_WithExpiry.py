...
IFunction[IOrderGenerator,IFunction[Side],IFunction[float]]):""" 
    ...
    def __repr__(self):return "sidevolume_WithExpiry(%(expiry)s, %(proto)s)" % self.__dict__
    
    def __call__(self, side = None,volume = None):from marketsim.gen._out.order._WithExpiry import WithExpiry
        expiry = self.expiry
        proto = self.proto
