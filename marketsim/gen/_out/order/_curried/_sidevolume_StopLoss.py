...
class sidevolume_StopLoss_FloatSideFloatIObservableIOrder(IFunctionIObservableIOrderIFunctionSideIFunctionfloat):
    """ 
...
def sidevolume_StopLoss(maxloss = None,proto = None): 
    ...
    if maxloss is None or rtti.can_be_casted(maxloss, IFunctionfloat):
        if proto is None or rtti.can_be_casted(proto, IFunctionIObservableIOrderIFunctionSideIFunctionfloat):
            return sidevolume_StopLoss_FloatSideFloatIObservableIOrder(maxloss,proto)
    raise Exception('Cannot find suitable overload for sidevolume_StopLoss('+str(maxloss) +':'+ str(type(maxloss))+','+str(proto) +':'+ str(type(proto))+')')
