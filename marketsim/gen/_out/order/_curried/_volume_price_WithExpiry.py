...
IFunction[IFunction[IOrderGenerator,IFunction[float]],IFunction[float]]):""" 
    ...
    def __repr__(self):return "volume_price_WithExpiry(%(expiry)s, %(proto)s)" % self.__dict__
    
    def __call__(self, volume = None):from marketsim.gen._out.order._curried._price_WithExpiry import price_WithExpiry
        expiry = self.expiry
        proto = self.proto
