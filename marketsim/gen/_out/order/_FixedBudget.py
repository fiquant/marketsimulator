...
class FixedBudget(IOrderGenerator, Observable[Order]):""" 
    ...
    """ 
    def __init__(self, side = None, budget = None):from marketsim import Order
        from marketsim.ops._all import Observable
        ...
        if isinstance(budget, types.IEvent):event.subscribe(self.budget, self.fire, self)
    ...
    def __repr__(self):return "FixedBudget(%(side)s, %(budget)s)" % self.__dict__
    
    def __call__(self, *args, **kwargs):from marketsim.gen._intrinsic.order.meta.fixed_budget import Order_Impl
        side = self.side()
