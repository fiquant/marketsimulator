...
package order
{
    ...
    def Market(side = side.Sell(), volume = constant(1.)) : IOrderGenerator

    @python.order.factory("order.limit.Order_Impl")
    def Limit(side = side.Sell(), price = constant(100.), volume = constant(1.)) : IOrderGenerator

    @python.order.factory("order.meta.fixed_budget.Order_Impl")
    def FixedBudget(side = side.Sell(), budget = constant(1000.)) : IOrderGenerator
    ...
    def FloatingPrice(floatingPrice = constant(10.), proto = _curried.price_Limit()) : IOrderGenerator
