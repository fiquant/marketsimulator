...
package strategy.position() {
    ...
    def Bollinger_linear(/** alpha parameter for exponentially weighted moving everage and variance */ alpha = 0.15,
                         ...
                         /** trader in question */ trader = trader.SingleProxy()) = DesiredPosition(observable.OnEveryDt(math.EW.RelStdDev(orderbook.MidPrice(orderbook.OfTrader(trader)),alpha)*k,1.0),trader)
    ...
    def RSI_linear(/** alpha parameter for exponentially moving averages of up movements and down movements */ alpha = 1.0/14.0,
                   ...
                   /** trader in question */ trader = trader.SingleProxy()) = DesiredPosition(observable.OnEveryDt((50.0-math.RSI(orderbook.OfTrader(trader),timeframe,alpha))*k,1.0),trader)
}
