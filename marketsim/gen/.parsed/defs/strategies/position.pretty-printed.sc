...
package strategy() {
    ...
    def RSI_linear(/** order factory function */ orderFactory = order.signedVolume.MarketSigned(),
                   ...
                   /** lag for calculating up and down movements */ timeframe = 1.0) = orderFactory(position.RSI_linear(alpha,k,timeframe))~>Strategy
    
    // defined at defs\strategies\position.sc: 17.5
    ...
    def Bollinger_linear(/** order factory function */ orderFactory = order.signedVolume.MarketSigned(),
                         ...
                         /** observable scaling function that maps relative deviation to desired position */ k = const(0.5)) = orderFactory(position.Bollinger_linear(alpha,k))~>Strategy
}
