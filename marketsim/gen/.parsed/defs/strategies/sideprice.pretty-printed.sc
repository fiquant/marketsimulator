...
package strategy() {
    ...
    def MarketData(/** Ticker of the asset */ ticker = "^GSPC",
                   ...
                   /** Volume of Buy/Sell orders. Should be large compared to the volumes of other traders. */ volume = 1000.0) = Combine(Generic(order.Iceberg(volume,order.FloatingPrice(ticker~>Quote(start,end)+delta~>BreaksAtChanges,order.price.Limit(side.Sell(),volume*1000))),event.After(0.0)),Generic(order.Iceberg(volume,order.FloatingPrice(ticker~>Quote(start,end)-delta~>BreaksAtChanges,order.price.Limit(side.Buy(),volume*1000))),event.After(0.0)))
    
    // defined at defs\strategies\sideprice.sc: 124.5
    def MarketMaker(delta = 1.0,
                    volume = 20.0) = Combine(Generic(order.Iceberg(volume,order.FloatingPrice(orderbook.Asks()~>SafeSidePrice(100+delta)/trader.Position()~>Atan/1000~>Exp~>OnEveryDt(0.9)~>BreaksAtChanges,order.price.Limit(side.Sell(),volume*1000))),event.After(0.0)),Generic(order.Iceberg(volume,order.FloatingPrice(orderbook.Bids()~>SafeSidePrice(100-delta)/trader.Position()~>Atan/1000~>Exp~>OnEveryDt(0.9)~>BreaksAtChanges,order.price.Limit(side.Buy(),volume*1000))),event.After(0.0)))
}
