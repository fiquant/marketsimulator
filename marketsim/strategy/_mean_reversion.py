...
class RelativeStrengthIndexSide(object):
    ...
    _properties = { 'rsi'    : meta.function((), Price),
...
def RSIEx    (          average              = mathutils.rsi(),
                       orderFactory          = order.MarketFactory, 
                       volumeDistr           = mathutils.rnd.expovariate(1.), 
                       creationIntervalDistr = mathutils.rnd.expovariate(1.)):
    ...
    rsi = observable.Fold(observable.Price(orderBook), average)
    
    r = Generic(orderFactory= orderFactory, 
                volumeFunc  = volumeDistr, 
                eventGen    = scheduler.Timer(creationIntervalDistr), 
                sideFunc    = RelativeStrengthIndexSide(orderBook, rsi))
