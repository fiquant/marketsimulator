...
class SidePriceVolumeLotSize(ops.Observable[types.SidePriceVolumeLotSize]): 
    
    def __init__(self, side, price, volume, lotsize):
        ops.Observable[types.SidePriceVolumeLotSize].__init__(self)
        self.side = side 
        self.price = price 
        self.volume = volume
        ...
        if isinstance(volume, Event):
            event.subscribe(volume, _(self).fire, self)
        if isinstance(lotsize, Event):
            event.subscribe(lotsize, _(self).fire, self)
    ...
    def __call__(self):
        ...
        return (side, price, volume, lotsize)
...
class SidePriceVolumeExpiry(ops.Observable[types.SidePriceVolumeExpiry]): 
    
    def __init__(self, side, price, volume, expiry):
        ops.Observable[types.SidePriceVolumeExpiry].__init__(self)
        self.side = side 
        self.price = price 
        self.volume = volume
        self.expiry = expiry
        ...
        if isinstance(volume, Event):
            event.subscribe(volume, _(self).fire, self)
        if isinstance(expiry, Event):
            event.subscribe(expiry, _(self).fire, self)
    ...
    def __call__(self):
        ...
        return (side, price, volume, expiry)
...
class SideVolumeMaxLoss(ops.Observable[types.SideVolumeMaxLoss]): 
    
    def __init__(self, side, volume, maxloss):
        ops.Observable[types.SideVolumeMaxLoss].__init__(self)
        self.side = side 
        self.volume = volume
        ...
        if isinstance(volume, Event):
            event.subscribe(volume, _(self).fire, self)
        if isinstance(maxloss, Event):
            event.subscribe(maxloss, _(self).fire, self)
